...
class reportes_ventas_forma_pago_xml
{
   var $Db;
   var $Erro;
   var $Ini;
...
function sc_actionbar_disable($buttonName)
{
   ...
   function inicializa_vars()
   {
      ...
      if (!is_file($this->Ini->root . $this->Ini->path_link . "reportes_ventas_forma_pago/reportes_ventas_forma_pago_res_xml.class.php"))
      {
          $this->Tem_xml_res  = false;
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['embutida'] && isset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_label']))
      {
          $this->Xml_tag_label = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_label'];
          $this->New_Format    = true;
      }
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_return']);
          if ($this->Tem_xml_res) {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "reportes_ventas_forma_pago.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name'] .= ".xml";
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      if (!$rs->EOF)
      {
         ...
         $this->fac_estado_sri = $rs->fields[15] ;  
         $this->fac_usuario = $rs->fields[16] ;  
         $this->fac_numero = $rs->fields[17] ;  
...
$_SESSION['scriptcase']['reportes_ventas_forma_pago']['contr_erro'] = 'off';
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['embutida_tit'] . ">\r\n";
         }
         elseif ($this->New_Format)
         {
             $this->xml_registro = "<reportes_ventas_forma_pago>\r\n";
         }
         else
         {
             $this->xml_registro = "<reportes_ventas_forma_pago";
...
$_SESSION['scriptcase']['reportes_ventas_forma_pago']['contr_erro'] = 'off'; 
      ...
      { 
          ...
          { 
              ...
              $this->Res = new reportes_ventas_forma_pago_res_xml();
              $this->prep_modulos("Res");
              $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_res_grid'] = true;
              $this->Res->monta_xml();
          } 
          if (!$this->Ini->sc_export_ajax) {
          ...
          { 
              ...
              { 
                  $str_zip   = "";
                  $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_res_file']['xml'];
                  ...
                  unlink($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_res_file']['xml']);
              }
              if ($this->Grava_view)
              {
                  $str_zip    = "";
                  ...
                  { 
                      $str_zip   = "";
                      $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_res_file']['view'];
                      $Arq_input = (FALSE !== strpos($Arq_res, ' ')) ? " \"" . $Arq_res . "\"" :  $Arq_res;
                      if (FALSE !== strpos(strtolower(php_uname()), 'windows')) 
                      {
                          $str_zip = "zip.exe " . strtoupper($Parm_pass) . " -j -u " . $this->Xml_password . " " . $Zip_f . " " . $Arq_input;
                      ...
                      unlink($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_res_file']['view']);
                  }
              } 
              else 
              {
              ...
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_res_grid']);
          } 
      }
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->fp_descripcion))
         {
             $this->fp_descripcion = sc_convert_encoding($this->fp_descripcion, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->Xml_tag_label)
         {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->fp_descripcion) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->fp_descripcion) . "\"";
         }
   }
   //----- fp_valor
   function NM_export_fp_valor()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->fp_valor) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->fp_valor) . "\"";
         }
   }
   //----- fac_comentario
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_facturas_forma_pago'] ?> :: XML</TITLE>
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['xml_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
