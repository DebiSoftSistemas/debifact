function grid_del_retencion_dos_ret_fecha_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_fecha" + seqRow).html();
}

function grid_del_retencion_dos_ret_fecha_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_fecha" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_establecimiento_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_establecimiento" + seqRow).html();
}

function grid_del_retencion_dos_ret_establecimiento_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_establecimiento" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_punto_emision_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_punto_emision" + seqRow).html();
}

function grid_del_retencion_dos_ret_punto_emision_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_punto_emision" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_secuencial_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_secuencial" + seqRow).html();
}

function grid_del_retencion_dos_ret_secuencial_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_secuencial" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_cliente_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_cliente" + seqRow).html();
}

function grid_del_retencion_dos_ret_cliente_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_cliente" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_mes_periodo_fiscal_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_mes_periodo_fiscal" + seqRow).html();
}

function grid_del_retencion_dos_ret_mes_periodo_fiscal_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_mes_periodo_fiscal" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_anio_periodo_fiscal_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_anio_periodo_fiscal" + seqRow).html();
}

function grid_del_retencion_dos_ret_anio_periodo_fiscal_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_anio_periodo_fiscal" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_estado_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_estado_sri" + seqRow).html();
}

function grid_del_retencion_dos_ret_estado_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_estado_sri" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_error_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_error_sri" + seqRow).html();
}

function grid_del_retencion_dos_ret_error_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_error_sri" + seqRow).html(newValue);
}

function grid_del_retencion_dos_ret_numero_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_ret_numero" + seqRow).html();
}

function grid_del_retencion_dos_ret_numero_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_ret_numero" + seqRow).html(newValue);
}

function grid_del_retencion_dos_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("ret_fecha" == field) {
    SC_tmp = grid_del_retencion_dos_ret_fecha_getValue(seqRow);
  }
  if ("ret_establecimiento" == field) {
    SC_tmp = grid_del_retencion_dos_ret_establecimiento_getValue(seqRow);
  }
  if ("ret_punto_emision" == field) {
    SC_tmp = grid_del_retencion_dos_ret_punto_emision_getValue(seqRow);
  }
  if ("ret_secuencial" == field) {
    SC_tmp = grid_del_retencion_dos_ret_secuencial_getValue(seqRow);
  }
  if ("ret_cliente" == field) {
    SC_tmp = grid_del_retencion_dos_ret_cliente_getValue(seqRow);
  }
  if ("ret_mes_periodo_fiscal" == field) {
    SC_tmp = grid_del_retencion_dos_ret_mes_periodo_fiscal_getValue(seqRow);
  }
  if ("ret_anio_periodo_fiscal" == field) {
    SC_tmp = grid_del_retencion_dos_ret_anio_periodo_fiscal_getValue(seqRow);
  }
  if ("ret_estado_sri" == field) {
    SC_tmp = grid_del_retencion_dos_ret_estado_sri_getValue(seqRow);
  }
  if ("ret_error_sri" == field) {
    SC_tmp = grid_del_retencion_dos_ret_error_sri_getValue(seqRow);
  }
  if ("ret_numero" == field) {
    SC_tmp = grid_del_retencion_dos_ret_numero_getValue(seqRow);
  }
...
function grid_del_retencion_dos_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("ret_fecha" == field) {
    grid_del_retencion_dos_ret_fecha_setValue(newValue, seqRow);
  }
  if ("ret_establecimiento" == field) {
    grid_del_retencion_dos_ret_establecimiento_setValue(newValue, seqRow);
  }
  if ("ret_punto_emision" == field) {
    grid_del_retencion_dos_ret_punto_emision_setValue(newValue, seqRow);
  }
  if ("ret_secuencial" == field) {
    grid_del_retencion_dos_ret_secuencial_setValue(newValue, seqRow);
  }
  if ("ret_cliente" == field) {
    grid_del_retencion_dos_ret_cliente_setValue(newValue, seqRow);
  }
  if ("ret_mes_periodo_fiscal" == field) {
    grid_del_retencion_dos_ret_mes_periodo_fiscal_setValue(newValue, seqRow);
  }
  if ("ret_anio_periodo_fiscal" == field) {
    grid_del_retencion_dos_ret_anio_periodo_fiscal_setValue(newValue, seqRow);
  }
  if ("ret_estado_sri" == field) {
    grid_del_retencion_dos_ret_estado_sri_setValue(newValue, seqRow);
  }
  if ("ret_error_sri" == field) {
    grid_del_retencion_dos_ret_error_sri_setValue(newValue, seqRow);
  }
  if ("ret_numero" == field) {
    grid_del_retencion_dos_ret_numero_setValue(newValue, seqRow);
  }
}
...
function scAjaxCall_actbtn_btn_anular_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_anular_onClick&script_case_init=" + document.F3.script_case_init.value + "&ret_numero=" + grid_del_retencion_dos_getValue("ret_numero", seqRow) + "&lang_doc_anulado=" + grid_del_retencion_dos_getValue("lang_doc_anulado", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_del_retencion_dos_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
...
function scAjaxCall_actbtn_btn_autorizar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_autorizar_onClick&script_case_init=" + document.F3.script_case_init.value + "&ret_numero=" + grid_del_retencion_dos_getValue("ret_numero", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      var i, fieldDisplay, oResp;
      Tst_integrid = jsonReturn.trim();
...
function ajax_navigate(opc, parm)
{
    ...
    return new Promise(function(resolve, reject) {$.ajax({
     ...
     .done(function(jsonNavigate) {
        ...
        if (!SC_Proc_Mobile) {
            document.getElementById('nmsc_iframe_liga_A_grid_del_retencion_dos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_E_grid_del_retencion_dos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_D_grid_del_retencion_dos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_B_grid_del_retencion_dos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_A_grid_del_retencion_dos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_retencion_dos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_retencion_dos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_retencion_dos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_A_grid_del_retencion_dos').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_retencion_dos').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_retencion_dos').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_retencion_dos').style.width  = '0px';
        }
        if (oResp["ajaxMessage"]) {
           nmAjaxMessage(oResp);
        }
