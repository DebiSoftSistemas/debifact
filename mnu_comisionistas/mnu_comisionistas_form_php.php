...
class mnu_comisionistas_form_php
{
      var $sc_script_name;
      var $nm_location;
   ...
   function init()
   {
      ...
      if(!isset($_SESSION['scriptcase']['mnu_comisionistas']['glo_nm_path_prod']) || empty($_SESSION['scriptcase']['mnu_comisionistas']['glo_nm_path_prod']))
      {
              /*check prod*/$_SESSION['scriptcase']['mnu_comisionistas']['glo_nm_path_prod'] = $str_path_apl_url . "_lib/prod";
      }
...
<?php
      ...
      if (isset($_GET['sc_apl_menu']))
      {
          ...
          $_SESSION['scriptcase']['sc_apl_menu_link'] = $_SESSION['scriptcase']['mnu_comisionistas']['sc_apl_link'];
...
//
      ...
      $_SESSION['scriptcase']['mnu_comisionistas']['contr_erro'] = 'on';
...
switch ($this->sc_menu_item){
	...
	case "item_3": $check_sql = "SELECT cat_id,cat_nombre FROM sis_catalogo where cat_id=82";
...
if ($this->Db)
{
    $this->Db->Close(); 
      ...
      {
         foreach($_SESSION['scriptcase']['sc_def_menu']['mnu_comisionistas'] as $id_item => $arr_item)
         {
      ...
      {
          $apl_run = $_SESSION['scriptcase']['sc_apl_menu_link'] . $this->tab_grupo[0] . SC_dir_app_name("form_sis_detalle_catalogo_configurador") . "/?nm_run_menu=1&nm_apl_menu=mnu_comisionistas&script_case_init=" . $this->Gera_sc_init($this->sc_menu_item) . "";
      }
      if ($_SESSION['scriptcase']['sc_item_menu'] == "item_2")
      {
          $apl_run = $_SESSION['scriptcase']['sc_apl_menu_link'] . $this->tab_grupo[0] . SC_dir_app_name("grid_del_comisionista") . "/?nm_run_menu=1&nm_apl_menu=mnu_comisionistas&script_case_init=" . $this->Gera_sc_init($this->sc_menu_item) . "";
      }
      }
      if (!$link_url)
      {
...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
      ...
      <html><body>
        <form name="fmenu" method="post" action="<?php echo NM_encode_input($apl_run); ?>">
          ...
          <input type=hidden name="nm_apl_menu" value="mnu_comisionistas"> 
<?php
      if (isset($_SESSION['scriptcase']['menu_mobile']) && $_SESSION['scriptcase']['menu_mobile'] == "mnu_comisionistas")
      {
?>
...
<?php
      ...
      {
?>
          window.history.pushState('Object', 'mnu_comisionistas', '<?php echo $this->nm_location ?>');
<?php
      }
      if ($link_url)
      {
...
$controle = new mnu_comisionistas_form_php();
$controle->init();
?>
