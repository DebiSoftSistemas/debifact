...
class chart_del_factura_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['under_dashboard'] ? "off" : "on";

      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_del_factura'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_del_factura'];
   ...
   if ($this->Ini->Embutida_iframe) {
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['sub_cons_iframe_btns'] as $BTN => $BTN_opc) {
           $this->nmgp_botoes[$BTN] = $BTN_opc;
       }
   }
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['chart_del_factura']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['chart_del_factura']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['chart_del_factura']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
...
SCEOT;
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_step']     = 'image';
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_step'] = 'end';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['chart_del_factura']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['campos_busca'];
         ...
         {
             ob_start();
             $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_combination_type'] = $_POST['fc_chart_type'];
         }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_y_axys']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_y_axys']))
       {
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_x_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_y_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_order'] = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['comb_field_display_type']  = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_display_values']     = array();

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_combination_type']        = 'column3d';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_combination_order_rule']  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_combination_order_field'] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_combination_order_rule_groupby']["fac_fecha"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_combination_order_field_groupby']["fac_fecha"] = 'label';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_y_axys'][0] = 0;
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['comb_field_display_type'][2] = 'bar';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_display_values'][2] = true;


           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_order']['sc_free_group_by'][] = 2;
       }

      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pesq_tab_label']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pesq_tab_label'] = "";
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_search_add']))
      {
          $this->monta_css();
          $nmgp_tab_label = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pesq_tab_label'];
          ...
          $Seq_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_search_add']['seq'];
          $Cmp_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_search_add']['cmp'];
          ...
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_search_add']);
          exit;
      }
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['res_hrz'];
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral(false, $this->NM_export);
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['tot_geral'][2];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['tot_geral'][1] == 0)
      {
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['arr_total']['fac_fecha'] = $this->array_total_fac_fecha;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['json_use_label'])
       {
       ...
       $Lab_fac_fecha =  sprintf("" . $this->Ini->Nm_lang['lang_srch_mnth'] . "", $Lab_fac_fecha);
       }
       $prep_field = array();
       $prep_field['fac_fecha'] = $Lab_fac_fecha;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_fecha'] = $Lab_fac_fecha; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_numero']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_numero'] = "Fac Numero"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_tipo_comprobante']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_tipo_comprobante'] = "Fac Tipo Comprobante"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_empresa']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_empresa'] = "Fac Empresa"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_establecimiento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_establecimiento'] = "Fac Establecimiento"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_punto_emision']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_punto_emision'] = "Fac Punto Emision"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_secuencial']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_secuencial'] = "Fac Secuencial"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_cliente']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_cliente'] = "Fac Cliente"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_fecha']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_fecha'] = "Fac Fecha"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_ambiente']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_ambiente'] = "Fac Ambiente"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_tipo_libretin']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_tipo_libretin'] = "Fac Tipo Libretin"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_iva']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_iva'] = "Fac Subtotal Iva"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_cero']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_cero'] = "Fac Subtotal Cero"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_no_objeto']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_no_objeto'] = "Fac Subtotal No Objeto"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_excento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal_excento'] = "Fac Subtotal Excento"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_subtotal'] = "Fac Subtotal"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_total_descuento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_total_descuento'] = "Fac Total Descuento"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_valor_ice']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_valor_ice'] = "Fac Valor Ice"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_valor_iva']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_valor_iva'] = "Fac Valor Iva"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_valor_irbpnr']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_valor_irbpnr'] = "Fac Valor Irbpnr"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_propina']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_propina'] = "Fac Propina"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_total']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_total'] = "Fac Total"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_estado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_estado'] = "Fac Estado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_estado_sri']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_estado_sri'] = "Fac Estado Sri"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_error_sri']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_error_sri'] = "Fac Error Sri"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_fecha_autorizacion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_fecha_autorizacion'] = "Fac Fecha Autorizacion"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_autorizacion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_autorizacion'] = "Fac Autorizacion"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_usuario']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_usuario'] = "Fac Usuario"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_serie_guia']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_serie_guia'] = "Fac Serie Guia"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_comentario']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_comentario'] = "Fac Comentario"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_aplica_propina']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_aplica_propina'] = "Fac Aplica Propina"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_libretin']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_libretin'] = "Fac Libretin"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_clave']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['labels']['fac_clave'] = "Fac Clave"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       //-----
       $this->comp_sum = array(
           2 => "" .  $this->Ini->Nm_lang['lang_othr_chrt_totl'] . "",
       );
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
       ...
       //-----
       $this->comp_sum_lnk = array(
           2 => array('field' => "fac_total", 'show' => true),
       );
      ...
      eval ($Str_gb);
       ...
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_del_factura@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['proc_pdf_vert'])
   {
     $this->comp_tab_hover = true;
   }
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           'fac_fecha' => array(
           '1' => array(
               'title'    => $this->getChartText(strip_tags("" .  $this->Ini->Nm_lang['lang_othr_chrt_totl'] . "")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'fac_fecha',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_fac_fecha)),
               'label_y'  => $this->getChartText(strip_tags("" .  $this->Ini->Nm_lang['lang_othr_chrt_totl'] . "")),
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_order']['sc_free_group_by'] as $i_summ_order)
       {
                   ...
                   foreach ($this->sc_comb_quick_index[$i_index_summ] as $i_index_comb_groupby)
                   {
                       ...
                       if (!isset($a_chart_combinations[$s_comb_index]))
                       {
                           $a_chart_combinations[$s_comb_index] = array(
                               ...
                               'label_x'     => '',
                               'label_y'     => '',
                               'xml_pholder' => sizeof($this->comp_y_axys) > $i_index_group + 1 ? 'sc_chart_del_factura_' . session_id() . '_!!!C_' . ($i_index_group + 1) . '_' . $s_index_comb . ('' == $s_index_comb ? '' : '_') . '__SC_PLACEHOLDER__!!!.json' : '',
                               'xml'         => 'sc_chart_del_factura_' . session_id() . '_!!!C_' . $i_index_group . '_' . $s_index_comb . '!!!.json',
                           );
                       }
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'];
	}
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
	...
	function filterChartsByMultiseries() {
		...
		if ($applyFilter && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_drill_down']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['summarizing_drill_down'])) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'] as $chartIndex => $chartData) {
				$indexParts = explode('|', $chartIndex);
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_chart_del_factura_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_chart_del_factura_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['embutida'])
                   {
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialOrderFA = '';
                       ...
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
      {
      ...
      { 
         ...
         { 
             $seq_search = 1;
             foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq'] as $cmp => $def)
             {
                $Seq_grid      = $seq_search;
                $Cmp_grid      = $cmp;
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_del_factura']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_del_factura']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_del_factura']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_del_factura']) . "_";
               } 
           }
       }
   ...
   function resumo_final()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['ajax_nav'])
      {
   ...
   {
      $this->Tot_ger = false;
      require_once($this->Ini->path_aplicacao . "chart_del_factura_total.class.php"); 
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      $this->Total   = new chart_del_factura_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_refresh_after_chart'] = 'resumo';
      $nm_saida->saida("<HTML" . $_SESSION['scriptcase']['reg_conf']['html_dir'] . ">\r\n");
      $nm_saida->saida("<HEAD>\r\n");
      $nm_saida->saida(" <TITLE>Total de Ventas</TITLE>\r\n");
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['doc_word'])
{
      ...
      { 
           ...
           if ($_SESSION['scriptcase']['proc_mobile'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['ajax_nav']) {  
        ...
        <link rel='stylesheet' href='../_lib/lib/css/nm_mobile.css' type='text/css'/>
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_del_factura_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_del_factura_jquery_1599.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_del_factura_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
...
$nm_saida->saida("    </style>\r\n");
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_del_factura_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_chart_del_factura_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_del_factura_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css'] . '.css');
     ...
     $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
     $nm_saida->saida(" <style type=\"text/css\">\r\n");
     if (!$this->Ini->Export_html_zip)     {
      ...
      $nm_saida->saida(" </style>\r\n");
           ...
           $nm_saida->saida("     }\r\n");
           $nm_saida->saida("   </script>\r\n");
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['ajax_nav'] && !$this->Ini->sc_export_ajax && !$this->Ini->Export_html_zip && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['doc_word'])
      {
           $nm_saida->saida("   <script type=\"text/javascript\"> \r\n");
          ...
          if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq']))
          { 
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['remove_margin'] ? 'margin: 0;' : '';
          $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['remove_border'] ? 'border-width: 0;' : '';
          $vertical_center = '';
          $nm_saida->saida(" <BODY id=\"grid_chart\" class=\"" . $this->css_scGridPage . " sc-app-chart\" style=\"" . $remove_margin . $vertical_center . "\">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['embutida'])
      {
...
;
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['embutida'])
      {
         ...
         if ($this->nmgp_botoes['chart_bar'] == 'on') {
            $Cod_Btn = nmButtonOutput($this->arr_buttons, "bchart_bar", "scChangeChartTypeButton('column2d');", "scChangeChartTypeButton('column2d');", "chart_bar_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "sc-chart-type-button", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
            $nm_saida->saida("     $Cod_Btn\r\n");
            $NM_btn = true;
         }
         $nm_saida->saida("     <script type=\"text/javascript\">\r\n");
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_config_pdf.php?nm_opc=pdf&nm_target=1&nm_cor=cor&papel=8&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=s&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&pdf_zip=N&origem=chart&language=es&conf_socor=N&is_chart_app=Y&nm_label_group=N&nm_all_cab=S&nm_all_label=S&nm_orient_grid=2&script_case_init=" . $this->Ini->sc_page . "&app_name=chart_del_factura&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
         if (is_file("chart_del_factura_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("chart_del_factura_help.txt"); 
   ...
   function monta_barra_top_mobile()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $this->nm_btn_exist['pdf'][] = "Rpdf_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_config_pdf.php?nm_opc=pdf&nm_target=1&nm_cor=cor&papel=8&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=s&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&pdf_zip=N&origem=chart&language=es&conf_socor=N&is_chart_app=Y&nm_label_group=N&nm_all_cab=S&nm_all_label=S&nm_orient_grid=2&script_case_init=" . $this->Ini->sc_page . "&app_name=chart_del_factura&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['word'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bword", "", "", "Rword_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_config_word.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_cor=CO&nm_res_cons=s&nm_ini_word_res=chart&nm_all_modules=chart&password=n&origem=chart&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['xls'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bexcel", "", "", "Rxls_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_config_xls.php?script_case_init=" . $this->Ini->sc_page . "&app_name=chart_del_factura&nm_tp_xls=xlsx&nm_tot_xls=S&nm_tot_xls=S&nm_res_cons=s&nm_ini_xls_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&origem=chart&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['xml'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bxml", "", "", "Rxml_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_config_xml.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&password=n&nm_res_cons=s&nm_ini_xml_res=chart&nm_all_modules=chart&nm_xml_tag=tag&nm_xml_label=S&language=es&origem=chart&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['csv'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bcsv", "", "", "Rcsv_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_config_csv.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_res_cons=s&nm_ini_csv_res=chart&nm_all_modules=chart&password=n&nm_delim_line=1&nm_delim_col=1&nm_delim_dados=1&nm_label_csv=N&language=es&origem=chart&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Rprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_config_print.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_opc=resumo&nm_cor=CO&nm_res_cons=s&nm_ini_prt_res=chart&nm_all_modules=chart&password=n&origem=chart&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
   ...
   function monta_html_fim()
   {
      ...
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_del_factura']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_del_factura']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
$nm_saida->saida("       scJs_alert('" . $mensagem . "', $jsonParams);\r\n");
      ...
      elseif ($this->Ini->Export_html_zip || ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['opcao'] == "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['graf_pdf'] == "S"))
      {
          $this->grafico_pdf();
      }
      else
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'chart_del_factura_config_pivot.php?nome_apl=chart_del_factura&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"chart_del_factura_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   }\r\n");
          ...
          $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "chart_del_factura/chart_del_factura_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\"+ str_type +\"&sAdd=__E__nmgp_cor_word=\" + cor + \"__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
          $nm_saida->saida("       }\r\n");
          $nm_saida->saida("       else\r\n");
          $nm_saida->saida("       {\r\n");
          ...
          $nm_saida->saida("               document.Fprint.action = \"chart_del_factura_export_ctrl.php\";\r\n");
          $nm_saida->saida("           }\r\n");
          $nm_saida->saida("           else\r\n");
          $nm_saida->saida("           {\r\n");
          ...
          $nm_saida->saida("       document.Fexport.action = \"chart_del_factura_export_ctrl.php\";\r\n");
      ...
      if (!extension_loaded("zip"))
      {
          ...
          $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
          $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_del_factura_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_del_factura@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
          ...
          $nm_saida->saida("      if (\"R\" == ajax)\r\n");
          $nm_saida->saida("      {\r\n");
          $nm_saida->saida("          ajax_export('pdf_res','&nmgp_tipo_pdf=' + z + '&sc_create_charts=' + crt + '&nmgp_graf_pdf=' + g + '&chart_level=' + chart_level, false);\r\n");
          $nm_saida->saida("      }\r\n");
          $nm_saida->saida("      else\r\n");
          $nm_saida->saida("      {\r\n");
          ...
          $nm_saida->saida("      if (\"R\" == ajax)\r\n");
          $nm_saida->saida("      {\r\n");
          $nm_saida->saida("          ajax_export('pdf_res','&nmgp_tipo_pdf=' + z + '&sc_create_charts=' + crt + '&nmgp_graf_pdf=' + g + '&chart_level=' + chart_level + '&SC_module_export=' + SC_module_export + '&use_pass_pdf=' + use_pass_pdf + '&pdf_all_cab=' + pdf_all_cab + '&pdf_all_label=' +  pdf_all_label + '&pdf_label_group=' +  pdf_label_group + \"&pdf_zip=\" +  pdf_zip, false);\r\n");
          $nm_saida->saida("      }\r\n");
          $nm_saida->saida("      else\r\n");
          $nm_saida->saida("      {\r\n");
          $nm_saida->saida("          document.Fres_pdf.nmgp_opcao.value = \"pdf\" ;\r\n");
...
;
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_del_factura_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "chart_del_factura/chart_del_factura_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['skip_charts']))
   {
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'] as $chart_index => $chart_data)
       {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               chart_del_factura_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['this_chart_label'] = '';
           $this->Graf->monta_grafico($chart_index, 'pdf');
           if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['this_chart_label']) {
               $nm_saida->saida("<script type=\"text/javascript\">$(\"#$random_id\").html(\"{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['this_chart_label']}\")</script>\r\n");
           }
       ...
       if ($bChartFP)
       {
           ...
           chart_del_factura_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_list']))
   {
       $prim_graf = true;
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['chart_list'] as $arr_chart)
       {
           if (!$prim_graf)
           {
               $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
...
//--- 
 function grafico_bot()
 {
   ...
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['graf_bot'] = true;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['graf_first'] = true;
       require_once($this->Ini->path_aplicacao . $this->Ini->Apl_grafico); 
       $this->Graf  = new chart_del_factura_grafico();
       $this->Graf->Db     = $this->Db;
       $this->Graf->Erro   = $this->Erro;
       ...
       $nm_saida->saida("<TR>\r\n");
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts']);
       $sChartLang  = isset($this->Ini->Nm_lang['lang_pdff_pcht']) ? $this->Ini->Nm_lang['lang_pdff_pcht'] : 'Generating chart';
       if (!NM_is_utf8($sChartLang))
       {
           $sChartLang = sc_convert_encoding($sChartLang, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      if ($bPDF && (!isset($this->progress_fp) || !$this->progress_fp))
      {
           ...
           chart_del_factura_pdf_progress_call("PDF\n", $this->Ini->Nm_lang);
           chart_del_factura_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_del_factura_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_del_factura_pdf_progress_call("100\n", $this->Ini->Nm_lang);
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['pivot_charts'] as $chart_index => $chart_data)
       {
           ...
           if ($bPDF)
           {
               ...
               if ($this->progress_fp)
               {
                   chart_del_factura_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "/" . $iChartTotal . "...\n", $this->Ini->Nm_lang, true);
       ...
       $nm_saida->saida("</TD>\r\n");
       $nm_saida->saida("</TR>\r\n");
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['graf_bot'] = false;
       if ($bPDF && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['charts_html'])
       {
           $nm_saida->saida("<script type=\"text/javascript\">\r\n");
           $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['charts_html']}\r\n");
           $nm_saida->saida("</script>\r\n");
...
//--- 
   ...
   {
      global $nm_saida;
      if ($this->Ini->Embutida_iframe || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['cond_pesq']))
      {  
          $pos       = 0;
          $trab_pos  = false;
          $pos_tmp   = true; 
          $tmp       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['cond_pesq'];
          while ($pos_tmp)
          {
             $pos = strpos($tmp, "##*@@", $pos);
             if ($pos !== false)
          ...
          $nm_cond_filtro_or  = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['cond_pesq'], $trab_pos + 5) == "or")  ? " " . trim($this->Ini->Nm_lang['lang_srch_orr_cond']) . " " : "";
          $nm_cond_filtro_and = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['cond_pesq'], $trab_pos + 5) == "and") ? " " . trim($this->Ini->Nm_lang['lang_srch_and_cond']) . " " : "";
          $nm_cab_filtro   = substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['cond_pesq'], 0, $trab_pos);
     ...
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['proc_pdf_vert']) {
      ...
      $nm_saida->saida("          Total de Ventas\r\n");
      $nm_saida->saida("        </TD>\r\n");
   ...
   //---- 
   function adiciona_registro($sum_fac_total, $quebra_fac_fecha, $quebra_fac_fecha_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
          ...
          {
              $this->' . $arr_name . '[0]++;
              $this->' . $arr_name . '[1] = bcadd($this->' . $arr_name . '[1], ' . $sum_fac_total . ', 10);
          }
          ');
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['where_pesq']; 
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->fac_numero = $rs_res->fields[0] ;  
            $this->fac_tipo_comprobante = $rs_res->fields[1] ;  
            $this->fac_empresa = $rs_res->fields[2] ;  
            $this->fac_establecimiento = $rs_res->fields[3] ;  
            $this->fac_punto_emision = $rs_res->fields[4] ;  
            $this->fac_secuencial = $rs_res->fields[5] ;  
            $this->fac_cliente = $rs_res->fields[6] ;  
            $this->fac_fecha = $rs_res->fields[7] ;  
            $this->fac_ambiente = $rs_res->fields[8] ;  
            $this->fac_tipo_libretin = $rs_res->fields[9] ;  
            $this->fac_subtotal_iva = $rs_res->fields[10] ;  
            $this->fac_subtotal_iva =  str_replace(",", ".", $this->fac_subtotal_iva);
            $this->fac_subtotal_cero = $rs_res->fields[11] ;  
            $this->fac_subtotal_cero =  str_replace(",", ".", $this->fac_subtotal_cero);
            $this->fac_subtotal_no_objeto = $rs_res->fields[12] ;  
            $this->fac_subtotal_no_objeto =  str_replace(",", ".", $this->fac_subtotal_no_objeto);
            $this->fac_subtotal_excento = $rs_res->fields[13] ;  
            $this->fac_subtotal_excento =  str_replace(",", ".", $this->fac_subtotal_excento);
            $this->fac_subtotal = $rs_res->fields[14] ;  
            $this->fac_subtotal =  str_replace(",", ".", $this->fac_subtotal);
            $this->fac_total_descuento = $rs_res->fields[15] ;  
            ...
            $this->fac_valor_iva = $rs_res->fields[17] ;  
            ...
            $this->fac_propina = $rs_res->fields[19] ;  
            $this->fac_propina =  str_replace(",", ".", $this->fac_propina);
            $this->fac_total = $rs_res->fields[20] ;  
            $this->fac_total =  str_replace(",", ".", $this->fac_total);
            $this->fac_estado = $rs_res->fields[21] ;  
            $this->fac_estado_sri = $rs_res->fields[22] ;  
            $this->fac_error_sri = $rs_res->fields[23] ;  
            $this->fac_fecha_autorizacion = $rs_res->fields[24] ;  
            $this->fac_autorizacion = $rs_res->fields[25] ;  
            $this->fac_usuario = $rs_res->fields[26] ;  
            $this->fac_serie_guia = $rs_res->fields[27] ;  
            $this->fac_comentario = $rs_res->fields[28] ;  
            $this->fac_aplica_propina = $rs_res->fields[29] ;  
            $this->fac_libretin = $rs_res->fields[30] ;  
            $this->fac_clave = $rs_res->fields[31] ;  
            $this->fac_fecha_orig = $this->fac_fecha;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->fac_total, $this->fac_fecha, $this->fac_fecha_orig);
            }
   }
//
   ...
   function html_grid_search()
   {
       ...
       $str_display = empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq'])?'none':'';
       if(!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['ajax_nav'])
       {
          $nm_saida->saida("   <tr id=\"NM_Grid_Search\" ajax='' style='display:" . $str_display . "'>\r\n");
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['ajax_nav'] && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq']))
       { 
           $_SESSION['scriptcase']['saida_html'] = "";
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq'] as $cmp => $def)
           {
           ...
           if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['save_grid']))
           {
               return;
           }
           else
       ...
       $nm_saida->saida("            <input type=\"hidden\" name=\"parm\" value=\"\"/> \r\n");
            if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq']))
            {
                foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq'] as $cmp => $def)
                {
                    if (isset($def['label'])) {
       ...
       { 
           ...
           $_SESSION['scriptcase']['saida_html'] = "";
           if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['save_grid']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq']))
           { 
   ...
   function grid_search_add_tag()
   {
       ...
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq']) && count($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq']) < 0)
       {
           $lin_obj .= "<table id='id_grid_search_all_cmp' cellpadding=0 cellspacing=0>";
           foreach ($All_cmp_search as $cada_cmp)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_del_factura']['grid_pesq'][$cada_cmp]))
               {
                   $lin_obj .= "  <tr>";
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "debi_doc_electronicos/chart_del_factura";
