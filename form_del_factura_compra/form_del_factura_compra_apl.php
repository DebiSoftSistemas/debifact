...
//---
   ...
   if (isset($this->Field_no_validate['liq_libretin'])) {
       return;
               ...
               if (!empty($this->liq_libretin) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura_compra']['Lookup_liq_libretin']) && !in_array($this->liq_libretin, $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura_compra']['Lookup_liq_libretin']))
               {
                   $hasError = true;
                   $Campos_Crit .= $this->Ini->Nm_lang['lang_errm_ajax_data'];
...
else
{
   ...
   $GLOBALS["NM_ERRO_IBASE"] = 0; 
          ...
          {
              $this->NM_ajax_info['fldList']['liq_libretin']['valList'][$i] = form_del_factura_compra_pack_protect_string($v);
          }
          foreach ($aLookupOrig as $aValData)
          {
