<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="fc_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['fc_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura_compra']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura_compra']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_factura_compra"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_retencion_var_id_factura_compra = "<?php echo $this->form_encode_input($this->nmgp_dados_form['fc_id']); ?>";
function sc_btn_retencion()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_retencion(); }, 500);
      return;
    }
    sc_btn_retencion_ok();
}
function sc_btn_retencion_cancel()
{
}
function sc_btn_retencion_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nueva_retencion; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_id_factura_compra*scin' + var_btn_retencion_var_id_factura_compra + '*scout"; ?>', '', '_self', '0', '0', 'blank_nueva_retencion');
}
...
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nueva_factura; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin09*scout"; ?>', '', '_self', '0', '0', 'blank_nueva_factura');
}
var var_btn_ride_var_id_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['fc_id']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_mostrar; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin03*scoutvar_id_documento*scin' + var_btn_ride_var_id_documento + '*scoutvar_extension_documento*scinpdf*scout"; ?>', '', '_blank', '0', '0', 'pdf_mostrar');
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura_compra']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_factura_compra_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_factura_compra_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("liq_secuencial=disabled;liq_tipo_libretin=disabled;ambiente=disabled;fc_ruc_proveedor=disabled;fc_nombre_proveedor=disabled;fc_direccion_proveedor=disabled;fc_email_proveedor=disabled;fc_subtotal=disabled;fc_subtotal_iva=disabled;fc_subtotal_cero=disabled;fc_subtotal_no_objeto=disabled;fc_subtotal_excento=disabled;fc_total_descuento=disabled;fc_valor_ice=disabled;fc_valor_iva=disabled;fc_valor_irbpnr=disabled;fc_propina=disabled;fc_total=disabled", "");
