function grid_del_factura_compra_fc_tipo_comprobante_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fc_tipo_comprobante" + seqRow).html();
}

function grid_del_factura_compra_fc_tipo_comprobante_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fc_tipo_comprobante" + seqRow).html(newValue);
}

function grid_del_factura_compra_sc_field_0_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_sc_field_0" + seqRow).html();
}

function grid_del_factura_compra_sc_field_0_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_sc_field_0" + seqRow).html(newValue);
}

function grid_del_factura_compra_fc_fecha_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fc_fecha" + seqRow).html();
}

function grid_del_factura_compra_fc_fecha_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fc_fecha" + seqRow).html(newValue);
}

function grid_del_factura_compra_pr_nombre_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_pr_nombre" + seqRow).html();
}

function grid_del_factura_compra_pr_nombre_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_pr_nombre" + seqRow).html(newValue);
}

function grid_del_factura_compra_pr_identificacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_pr_identificacion" + seqRow).html();
}

function grid_del_factura_compra_pr_identificacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_pr_identificacion" + seqRow).html(newValue);
}

function grid_del_factura_compra_fc_total_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fc_total" + seqRow).html();
}

function grid_del_factura_compra_fc_total_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fc_total" + seqRow).html(newValue);
}

function grid_del_factura_compra_liq_estado_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_liq_estado_sri" + seqRow).html();
}

function grid_del_factura_compra_liq_estado_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_liq_estado_sri" + seqRow).html(newValue);
}

function grid_del_factura_compra_fc_autorizacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fc_autorizacion" + seqRow).html();
}

function grid_del_factura_compra_fc_autorizacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fc_autorizacion" + seqRow).html(newValue);
}

function grid_del_factura_compra_liq_error_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_liq_error_sri" + seqRow).html();
}

function grid_del_factura_compra_liq_error_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_liq_error_sri" + seqRow).html(newValue);
}

function grid_del_factura_compra_fc_id_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fc_id" + seqRow).html();
}

function grid_del_factura_compra_fc_id_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fc_id" + seqRow).html(newValue);
}

function grid_del_factura_compra_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("fc_tipo_comprobante" == field) {
    SC_tmp = grid_del_factura_compra_fc_tipo_comprobante_getValue(seqRow);
  }
  if ("sc_field_0" == field) {
    SC_tmp = grid_del_factura_compra_sc_field_0_getValue(seqRow);
  }
  if ("fc_fecha" == field) {
    SC_tmp = grid_del_factura_compra_fc_fecha_getValue(seqRow);
  }
  if ("pr_nombre" == field) {
    SC_tmp = grid_del_factura_compra_pr_nombre_getValue(seqRow);
  }
  if ("pr_identificacion" == field) {
    SC_tmp = grid_del_factura_compra_pr_identificacion_getValue(seqRow);
  }
  if ("fc_total" == field) {
    SC_tmp = grid_del_factura_compra_fc_total_getValue(seqRow);
  }
  if ("liq_estado_sri" == field) {
    SC_tmp = grid_del_factura_compra_liq_estado_sri_getValue(seqRow);
  }
  if ("fc_autorizacion" == field) {
    SC_tmp = grid_del_factura_compra_fc_autorizacion_getValue(seqRow);
  }
  if ("liq_error_sri" == field) {
    SC_tmp = grid_del_factura_compra_liq_error_sri_getValue(seqRow);
  }
  if ("fc_id" == field) {
    SC_tmp = grid_del_factura_compra_fc_id_getValue(seqRow);
  }
...
function grid_del_factura_compra_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("fc_tipo_comprobante" == field) {
    grid_del_factura_compra_fc_tipo_comprobante_setValue(newValue, seqRow);
  }
  if ("sc_field_0" == field) {
    grid_del_factura_compra_sc_field_0_setValue(newValue, seqRow);
  }
  if ("fc_fecha" == field) {
    grid_del_factura_compra_fc_fecha_setValue(newValue, seqRow);
  }
  if ("pr_nombre" == field) {
    grid_del_factura_compra_pr_nombre_setValue(newValue, seqRow);
  }
  if ("pr_identificacion" == field) {
    grid_del_factura_compra_pr_identificacion_setValue(newValue, seqRow);
  }
  if ("fc_total" == field) {
    grid_del_factura_compra_fc_total_setValue(newValue, seqRow);
  }
  if ("liq_estado_sri" == field) {
    grid_del_factura_compra_liq_estado_sri_setValue(newValue, seqRow);
  }
  if ("fc_autorizacion" == field) {
    grid_del_factura_compra_fc_autorizacion_setValue(newValue, seqRow);
  }
  if ("liq_error_sri" == field) {
    grid_del_factura_compra_liq_error_sri_setValue(newValue, seqRow);
  }
  if ("fc_id" == field) {
    grid_del_factura_compra_fc_id_setValue(newValue, seqRow);
  }
}
...
function scAjaxCall_actbtn_btn_anular_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_anular_onClick&script_case_init=" + document.F3.script_case_init.value + "&fc_id=" + grid_del_factura_compra_getValue("fc_id", seqRow) + "&lang_doc_anulado=" + grid_del_factura_compra_getValue("lang_doc_anulado", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_del_factura_compra_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
...
function scAjaxCall_actbtn_btn_autorizar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_autorizar_onClick&script_case_init=" + document.F3.script_case_init.value + "&fc_id=" + grid_del_factura_compra_getValue("fc_id", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      var i, fieldDisplay, oResp;
      Tst_integrid = jsonReturn.trim();
...
function ajax_navigate(opc, parm)
{
    ...
    return new Promise(function(resolve, reject) {$.ajax({
     ...
     .done(function(jsonNavigate) {
        ...
        if (!SC_Proc_Mobile) {
            document.getElementById('nmsc_iframe_liga_A_grid_del_factura_compra').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_E_grid_del_factura_compra').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_D_grid_del_factura_compra').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_B_grid_del_factura_compra').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_A_grid_del_factura_compra').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_factura_compra').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_factura_compra').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_factura_compra').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_A_grid_del_factura_compra').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_factura_compra').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_factura_compra').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_factura_compra').style.width  = '0px';
        }
        if (oResp["ajaxMessage"]) {
           nmAjaxMessage(oResp);
        }
