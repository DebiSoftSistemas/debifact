<?php
include_once('mnu_pedidos_session.php');
...
if ($_SESSION['scriptcase']['device_mobile'])
{
    ...
    $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod']      = "";
    $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_perfil']         = "igtech_conexion";
    $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_imag_temp'] = "";
    $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo']      = "";
    ...
    //check prod
    if(empty($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod']))
    {
            /*check prod*/$_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'] = $str_path_apl_url . "_lib/prod";
    }
    //check tmp
    if(empty($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_imag_temp']))
    {
            /*check tmp*/$_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
    }
    //end check publication with the prod

ob_start();

class mnu_pedidos_class
{
  var $Db;
 ...
 function mnu_pedidos_menu()
 {
    global $mnu_pedidos_menuData, $nm_data_fixa;
     ...
     { 
         $_SESSION['scriptcase']['str_schema_all'] = $_POST["nmgp_schema"] . "/" . $_POST["nmgp_schema"];
           ...
           $nm_versao_sc  = "" ; 
           $_SESSION['scriptcase']['mnu_pedidos']['contr_erro'] = 'off';
           $Campos_Mens_erro = "";
...
$path_libs      = $str_root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'] . "/lib/php";
$path_third     = $str_root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'] . "/third";
$path_adodb     = $str_root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'] . "/third/adodb";
$path_apls      = $str_root . substr($path_link, 0, strrpos($path_link, '/'));
$path_img_old   = $str_root . $path_link . "mnu_pedidos/img";
$this->path_css = $str_root . $path_link . "_lib/css/";
$_SESSION['scriptcase']['dir_temp'] = $str_root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_imag_temp'];
...
$this->sc_Include($path_libs . "/nm_ini_perfil.php", "F", "perfil_lib") ; 
 if(function_exists('set_php_timezone')) set_php_timezone('mnu_pedidos');
...
if (!defined("SC_ERROR_HANDLER"))
{
    ...
    include_once(dirname(__FILE__) . "/mnu_pedidos_erro.php");
}
include_once(dirname(__FILE__) . "/mnu_pedidos_erro.class.php"); 
$this->Erro = new mnu_pedidos_erro();
$str_path = substr($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'], 0, strrpos($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'], '/') + 1);
if (!is_file($str_root . $str_path . 'devel/class/xmlparser/nmXmlparserIniSys.class.php'))
{
    unset($_SESSION['scriptcase']['nm_sc_retorno']);
    unset($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao']);
}

/* Definiciones de las rutas */
$mnu_pedidos_menuData         = array();
$mnu_pedidos_menuData['path'] = array();
$mnu_pedidos_menuData['url']  = array();
$mnu_pedidos_menuData['data'] = "";
$NM_dir_atual = getcwd();
if (empty($NM_dir_atual))
{
    $mnu_pedidos_menuData['path']['sys'] = (isset($_SERVER['SCRIPT_FILENAME'])) ? $_SERVER['SCRIPT_FILENAME'] : $_SERVER['ORIG_PATH_TRANSLATED'];
    $mnu_pedidos_menuData['path']['sys'] = str_replace("\\", '/', $str_path_sys);
    $mnu_pedidos_menuData['path']['sys'] = str_replace('//', '/', $str_path_sys);
}
else
{
    $sc_nm_arquivo                                   = explode("/", $_SERVER['PHP_SELF']);
    $mnu_pedidos_menuData['path']['sys'] = str_replace("\\", "/", str_replace("\\\\", "\\", getcwd())) . "/" . $sc_nm_arquivo[count($sc_nm_arquivo)-1];
}
$mnu_pedidos_menuData['url']['web']   = $_SERVER['PHP_SELF'];
$mnu_pedidos_menuData['url']['web']   = str_replace("\\", '/', $mnu_pedidos_menuData['url']['web']);
$mnu_pedidos_menuData['path']['root'] = substr($mnu_pedidos_menuData['path']['sys'],  0, -1 * strlen($mnu_pedidos_menuData['url']['web']));
$mnu_pedidos_menuData['path']['app']  = substr($mnu_pedidos_menuData['path']['sys'],  0, strrpos($mnu_pedidos_menuData['path']['sys'],  '/'));
$mnu_pedidos_menuData['path']['link'] = substr($mnu_pedidos_menuData['path']['app'],  0, strrpos($mnu_pedidos_menuData['path']['app'],  '/'));
$mnu_pedidos_menuData['path']['link'] = substr($mnu_pedidos_menuData['path']['link'], 0, strrpos($mnu_pedidos_menuData['path']['link'], '/')) . '/';
$mnu_pedidos_menuData['path']['app'] .= '/';
$mnu_pedidos_menuData['url']['app']   = substr($mnu_pedidos_menuData['url']['web'],  0, strrpos($mnu_pedidos_menuData['url']['web'],  '/'));
$mnu_pedidos_menuData['url']['link']  = substr($mnu_pedidos_menuData['url']['app'],  0, strrpos($mnu_pedidos_menuData['url']['app'],  '/'));
if ($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] == "S")
{
    $mnu_pedidos_menuData['url']['link']  = substr($mnu_pedidos_menuData['url']['link'], 0, strrpos($mnu_pedidos_menuData['url']['link'], '/'));
}
$mnu_pedidos_menuData['url']['link']  .= '/';
$mnu_pedidos_menuData['url']['app']   .= '/';


$_SESSION['scriptcase']['mnu_pedidos']['sc_apl_link'] = $mnu_pedidos_menuData['url']['link'];
...
if (isset($_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['lang'])) {
    $this->str_lang = $_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['lang'];
}
elseif (!isset($_SESSION['scriptcase']['mnu_pedidos']['actual_lang']) || $_SESSION['scriptcase']['mnu_pedidos']['actual_lang'] != $this->str_lang) {
    $_SESSION['scriptcase']['mnu_pedidos']['actual_lang'] = $this->str_lang;
...
include_once("mnu_pedidos_toolbar.php");
...
if ($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] != "S")
{
    $this->tab_grupo[0] = "";
     ...
     $_SESSION['scriptcase']['menu_atual'] = "mnu_pedidos";
     $_SESSION['scriptcase']['menu_apls']['mnu_pedidos'] = array();
     if (isset($_SESSION['scriptcase']['sc_connection']) && !empty($_SESSION['scriptcase']['sc_connection']))
     {
         foreach ($_SESSION['scriptcase']['sc_connection'] as $NM_con_orig => $NM_con_dest)
         {
             if (isset($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao']) && $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao'] == $NM_con_orig)
             {
/*NM*/           $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao'] = $NM_con_dest;
             }
             if (isset($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_perfil']) && $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_perfil'] == $NM_con_orig)
             {
/*NM*/           $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_perfil'] = $NM_con_dest;
             }
             if (isset($_SESSION['scriptcase']['mnu_pedidos']['glo_con_' . $NM_con_orig]))
             {
                 $_SESSION['scriptcase']['mnu_pedidos']['glo_con_' . $NM_con_orig] = $NM_con_dest;
             }
         }
     }
...
if (isset($_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir'])) {
...
';
    ...
    if ($_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir_tp'] == "R") {
        $SS_cod_html .= "  </HEAD>\r\n";
        $SS_cod_html .= "   <body>\r\n";
    }
    else {
        ...
        $SS_cod_html .= "           <input type=\"button\" name=\"sc_sai_seg\" value=\"OK\" onclick=\"sc_session_redir('" . $_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir'] . "');\">\r\n";
        $SS_cod_html .= "     </form>\r\n";
    ...
    if ($_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir_tp'] == "R") {
        $SS_cod_html .= "      sc_session_redir('" . $_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir'] . "');\r\n";
    }
    ...
    $SS_cod_html .= " </body>\r\n";
    $SS_cod_html .= "</HTML>\r\n";
    unset($_SESSION['scriptcase']['mnu_pedidos']['session_timeout']);
...
{
    $_SESSION['sc_session'][1]['mnu_pedidos']['init'] = $script_case_init;
}
else
if (!isset($_SESSION['sc_session'][1]['mnu_pedidos']['init']))
{
    $_SESSION['sc_session'][1]['mnu_pedidos']['init'] = "";
}
$script_case_init = $_SESSION['sc_session'][1]['mnu_pedidos']['init'];
...
if (isset($_SESSION['sc_session']['SC_parm_violation']) && !isset($_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir']))
{
...
{ 
    $nm_url_saida = $_SESSION['scriptcase']['sc_saida_mnu_pedidos'];
}
elseif (!empty($nm_url_saida))
{
    $_SESSION['scriptcase']['sc_url_saida'][$script_case_init]  = $nm_url_saida;
    $_SESSION['scriptcase']['sc_saida_mnu_pedidos'] = $nm_url_saida;
}
else
{
    $_SESSION['scriptcase']['sc_saida_mnu_pedidos'] = (isset($_SERVER['HTTP_REFERER']) && !empty($_SERVER['HTTP_REFERER'])) ? $_SERVER['HTTP_REFERER'] : "javascript:window.close()";
}
...
{
    if(function_exists("nm_check_perfil_exists")) nm_check_perfil_exists($path_libs, $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod']);
    $_SESSION['sc_session'][1]['SC_Check_Perfil'] = true;
}
$nm_falta_var    = ""; 
$nm_falta_var_db = ""; 
if (isset($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao']) && !empty($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao']))
{
    db_conect_devel($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao'], $str_root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'], 'debi_doc_electronicos', 2); 
}
if (isset($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_perfil']) && !empty($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_perfil']))
{
   $_SESSION['scriptcase']['glo_perfil'] = $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_perfil'];
}
...
if (isset($_SESSION['scriptcase']['glo_date_separator']) && !empty($_SESSION['scriptcase']['glo_date_separator']))
{
    ...
    if (strlen($SC_temp) == 2)
    {
       $_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date']  = substr($SC_temp, 0, 1); 
       $_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date1'] = substr($SC_temp, 1, 1); 
   }
   else
    {
       $_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date']  = $SC_temp; 
       $_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date1'] = $SC_temp; 
   }
}
...
if (!empty($nm_falta_var) || !empty($nm_falta_var_db))
{
    ...
    { 
        if (isset($_SESSION['sc_session'][1]['mnu_pedidos']['sc_outra_jan']) && $_SESSION['sc_session'][1]['mnu_pedidos']['sc_outra_jan'])
        {
...
$_SESSION['scriptcase']['nm_bases_security']  = "enc_nm_enc_";
 $glo_senha_protect = (isset($_SESSION['scriptcase']['glo_senha_protect'])) ? $_SESSION['scriptcase']['glo_senha_protect'] : "S";
if (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && isset($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao']) && !empty($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao']))
{ 
   $this->Db = db_conect_devel($_SESSION['scriptcase']['mnu_pedidos']['glo_nm_conexao'], $str_root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'], 'debi_doc_electronicos'); 
} 
else 
{ 
...
if (in_array(strtolower($nm_tpbanco), $this->nm_bases_oracle))
{
   ...
   $_SESSION['sc_session'][$this->Ini->sc_page]['mnu_pedidos']['decimal_db'] = ".";
} 
//
$this->NM_gera_log_insert("Scriptcase", "access");
/* Dados do menu em sessao */
$_SESSION['nm_menu'] = array('prod' => $str_root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'] . '/third/COOLjsMenu/',
                              'url' => $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod'] . '/third/COOLjsMenu/');

if ((isset($nmgp_outra_jan) && $nmgp_outra_jan == "true") || (isset($_SESSION['scriptcase']['sc_outra_jan']) && $_SESSION['scriptcase']['sc_outra_jan'] == 'mnu_pedidos'))
{
    $_SESSION['sc_session'][1]['mnu_pedidos']['sc_outra_jan'] = true;
     unset($_SESSION['scriptcase']['sc_outra_jan']);
    $_SESSION['scriptcase']['sc_saida_mnu_pedidos'] = "javascript:window.close()";
}
/* Menú de configuración de las variables */
$mnu_pedidos_menuData['iframe'] = TRUE;

if (!isset($_SESSION['scriptcase']['sc_apl_seg']))
{
    $_SESSION['scriptcase']['sc_apl_seg'] = array();
}
if (is_file($path_apls . $this->tab_grupo[0] . '_lib/_app_data/form_del_config_tienda_ini.php'))
{
    require($path_apls . $this->tab_grupo[0] .'_lib/_app_data/form_del_config_tienda_ini.php');
    ...
    {
        if (!isset($_SESSION['scriptcase']['sc_apl_seg']['form_del_config_tienda']))
        {
...
if (is_file($path_apls . $this->tab_grupo[0] . '_lib/_app_data/blank_abrir_adm_wp_ini.php'))
{
    require($path_apls . $this->tab_grupo[0] .'_lib/_app_data/blank_abrir_adm_wp_ini.php');
    if ((!isset($arr_data['status']) || trim($arr_data['status']) == "NAO") || (isset($_SESSION['nm_session']['user']['sec']['flag']) && $_SESSION['nm_session']['user']['sec']['flag'] == "N")) 
    {
        if (!isset($_SESSION['scriptcase']['sc_apl_seg']['blank_abrir_adm_wp']))
        {
            $_SESSION['scriptcase']['sc_apl_seg']['blank_abrir_adm_wp'] = "on";
        }
    }
    if (isset($_SESSION['nm_session']['user']['sec']['flag']) && $_SESSION['nm_session']['user']['sec']['flag'] == "N") 
    { 
...
if (is_file($path_apls . $this->tab_grupo[0] . '_lib/_app_data/tabs_pedidos_ini.php'))
{
    require($path_apls . $this->tab_grupo[0] .'_lib/_app_data/tabs_pedidos_ini.php');
    if ((!isset($arr_data['status']) || trim($arr_data['status']) == "NAO") || (isset($_SESSION['nm_session']['user']['sec']['flag']) && $_SESSION['nm_session']['user']['sec']['flag'] == "N")) 
    {
        if (!isset($_SESSION['scriptcase']['sc_apl_seg']['tabs_pedidos']))
        {
...
$saida_apl = $_SESSION['scriptcase']['sc_saida_mnu_pedidos'];
$mnu_pedidos_menuData['data'] .= "item_1|.|" . $this->Nm_lang['lang_configuracion'] . "||" . $nm_var_hint[0] . "||_self|\n";
if (isset($_SESSION['scriptcase']['sc_apl_seg']['form_del_config_tienda']) && strtolower($_SESSION['scriptcase']['sc_apl_seg']['form_del_config_tienda']) == "on")
{
    $mnu_pedidos_menuData['data'] .= "item_2|..|" . $this->Nm_lang['lang_datos_tienda'] . "|mnu_pedidos_form_php.php?sc_item_menu=item_2&sc_apl_menu=form_del_config_tienda&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . "|" . $nm_var_hint[1] . "||" . $this->mnu_pedidos_target('_self') . "|" . "\n";
}
else
{
    $mnu_pedidos_menuData['data'] .= "item_2|..|" . $this->Nm_lang['lang_datos_tienda'] . "||||_self|disabled\n";
}
if (isset($_SESSION['scriptcase']['sc_apl_seg']['blank_abrir_adm_wp']) && strtolower($_SESSION['scriptcase']['sc_apl_seg']['blank_abrir_adm_wp']) == "on")
{
    $mnu_pedidos_menuData['data'] .= "item_6|..|" . $this->Nm_lang['lang_tienda_wp'] . "|mnu_pedidos_form_php.php?sc_item_menu=item_6&sc_apl_menu=blank_abrir_adm_wp&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . "|" . $nm_var_hint[2] . "||" . $this->mnu_pedidos_target('_self') . "|" . "\n";
}
else
{
    $mnu_pedidos_menuData['data'] .= "item_6|..|" . $this->Nm_lang['lang_tienda_wp'] . "||||_self|disabled\n";
}
if (isset($_SESSION['scriptcase']['sc_apl_seg']['tabs_pedidos']) && strtolower($_SESSION['scriptcase']['sc_apl_seg']['tabs_pedidos']) == "on")
{
    $mnu_pedidos_menuData['data'] .= "item_4|.|" . $this->Nm_lang['lang_pedidos_convenios'] . "|mnu_pedidos_form_php.php?sc_item_menu=item_4&sc_apl_menu=tabs_pedidos&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . "|" . $nm_var_hint[3] . "||" . $this->mnu_pedidos_target('_self') . "|" . "\n";
}
else
{
    $mnu_pedidos_menuData['data'] .= "item_4|.|" . $this->Nm_lang['lang_pedidos_convenios'] . "||||_self|disabled\n";
}
$mnu_pedidos_menuData['data'] .= "item_5|.|" . $this->Nm_lang['lang_reporte'] . "||" . $nm_var_hint[4] . "||_self|\n";
...
$mnu_pedidos_menuData['data'] = array();
$str_disabled = "N";
$str_link = "#";
$str_icon = "";
$icon_aba = "";
...
$str_disabled = "N";
$str_link = "mnu_pedidos_form_php.php?sc_item_menu=item_2&sc_apl_menu=form_del_config_tienda&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . "";
if (!isset($_SESSION['scriptcase']['sc_apl_seg']['form_del_config_tienda']) || strtolower($_SESSION['scriptcase']['sc_apl_seg']['form_del_config_tienda']) != "on")
{
    $str_link = "#";
    $str_disabled = "Y";
    ...
    $mnu_pedidos_menuData['data'][] = array(
        'label'    => "" . $this->Nm_lang['lang_datos_tienda'] . "",
        'level'    => "1",
        'link'     => $str_link,
        'hint'     => "" . $nm_var_hint[1] . "",
        'id'       => "item_2",
        ...
        'icon_aba' => $icon_aba,
        'icon_aba_inactive' => $icon_aba_inactive,
        'target'   => " item-target=\"" . $this->mnu_pedidos_target('_self') . "\"",
        'sc_id'    => "item_2",
        'disabled' => $str_disabled,
        'display'     => "text_img",
        'display_position'=> "text_right",
...
$str_disabled = "N";
$str_link = "mnu_pedidos_form_php.php?sc_item_menu=item_6&sc_apl_menu=blank_abrir_adm_wp&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . "";
if (!isset($_SESSION['scriptcase']['sc_apl_seg']['blank_abrir_adm_wp']) || strtolower($_SESSION['scriptcase']['sc_apl_seg']['blank_abrir_adm_wp']) != "on")
{
    $str_link = "#";
    $str_disabled = "Y";
    ...
    $mnu_pedidos_menuData['data'][] = array(
        'label'    => "" . $this->Nm_lang['lang_tienda_wp'] . "",
        'level'    => "1",
        'link'     => $str_link,
        'hint'     => "" . $nm_var_hint[2] . "",
        'id'       => "item_6",
...
$str_disabled = "N";
$str_link = "mnu_pedidos_form_php.php?sc_item_menu=item_4&sc_apl_menu=tabs_pedidos&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . "";
if (!isset($_SESSION['scriptcase']['sc_apl_seg']['tabs_pedidos']) || strtolower($_SESSION['scriptcase']['sc_apl_seg']['tabs_pedidos']) != "on")
{
    $str_link = "#";
    $str_disabled = "Y";
...
if (isset($_SESSION['scriptcase']['sc_def_menu']['mnu_pedidos']))
{
    $arr_menu_usu = $this->nm_arr_menu_recursiv($_SESSION['scriptcase']['sc_def_menu']['mnu_pedidos']);
    $this->nm_gera_menus($str_menu_usu, $arr_menu_usu, 1, 'mnu_pedidos');
    $mnu_pedidos_menuData['data'] = $str_menu_usu;
}
if (is_file("mnu_pedidos_help.txt"))
{
    $Arq_WebHelp = file("mnu_pedidos_help.txt"); 
        ...
        foreach ($Tmp as $Cada_help)
        {
            ...
            if (!empty($Tmp1[0]) && isset($Tmp1[1]) && !empty($Tmp1[1]) && $Tmp1[0] == "menu" && is_file($str_root . $path_help . $Tmp1[1]))
            {
                ...
                $mnu_pedidos_menuData['data'][] = array(
                    ...
                    'icon_aba' => $icon_aba,
                    'icon_aba_inactive' => $icon_aba_inactive,
                    'target'   => "" . $this->mnu_pedidos_target('_blank') . "",
                    'sc_id'    => "item_Help",
                    'disabled' => $str_disabled,
                    'display'     => "text",
                    'display_position'=> "",
...
if (isset($_SESSION['scriptcase']['sc_menu_del']['mnu_pedidos']) && !empty($_SESSION['scriptcase']['sc_menu_del']['mnu_pedidos']))
{
    $nivel = 0;
    $exclui_menu = false;
    foreach ($mnu_pedidos_menuData['data'] as $i_menu => $cada_menu)
    {
       if (in_array($cada_menu['id'], $_SESSION['scriptcase']['sc_menu_del']['mnu_pedidos']))
       {
          $nivel = $cada_menu['level'];
          $exclui_menu = true;
          unset($mnu_pedidos_menuData['data'][$i_menu]);
       }
    ...
    $mnu_pedidos_menuData['data'] = $Temp_menu;
}

if (isset($_SESSION['scriptcase']['sc_menu_disable']['mnu_pedidos']) && !empty($_SESSION['scriptcase']['sc_menu_disable']['mnu_pedidos']))
{
    $disable_menu = false;
    foreach ($mnu_pedidos_menuData['data'] as $i_menu => $cada_menu)
    {
       if (in_array($cada_menu['id'], $_SESSION['scriptcase']['sc_menu_disable']['mnu_pedidos']))
       {
          $nivel = $cada_menu['level'];
          $disable_menu = true;
          $mnu_pedidos_menuData['data'][$i_menu]['disabled'] = 'Y';
       }
...
if($this->force_mobile || ($_SESSION['scriptcase']['device_mobile'] && $_SESSION['scriptcase']['display_mobile']))
{
    $_SESSION['scriptcase']['menu_mobile'] = "mnu_pedidos";
    include('mnu_pedidos_mobile.php');
    exit;
}
/* Cabecera HTML */
if ($mnu_pedidos_menuData['iframe'])
{
    $mnu_pedidos_menuData['height'] = '100%';
...
<head>
 ...
 <link rel="stylesheet" href="<?php echo $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod']; ?>/third/font-awesome/css/all.min.css" type="text/css" media="screen" />
...
{
    $mnu_pedidos_menuData['height'] = '30px';
}

/* Archivos JS */
?>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod']; ?>/third/jquery/js/jquery.js"></script>
<script type="text/javascript" src="../_lib/lib/js/menu_structure.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
<script type="text/javascript" src="mnu_pedidos_message.js"></script>
...
Iframe_atual = "mnu_pedidos_iframe";
...
{
  if (str_id != "iframe_mnu_pedidos")
  {
      str_id        = str_id.replace("mnu_pedidos_","");
  }
  ...
  if (typeof str_link !== typeof undefined && str_link !== false) {
    ...
    if(str_target != '_blank' && $('#iframe_mnu_pedidos').length)
        $('#iframe_mnu_pedidos')[0].contentWindow.focus();
  }
}
</script>
<?php
...
<?php echo ($this->menu_orientacao=='vertical')?$this->nm_show_toolbarmenu($col_span, $saida_apl, $mnu_pedidos_menuData, $path_imag_cab):''; ?>  <tr class="scMenuHTableCssAlt" id='idMenuLine'>
  <?php
  if($this->menu_orientacao != 'vertical')
  {
    ?>
...
echo $this->mnu_pedidos_escreveMenu($mnu_pedidos_menuData['data'], $path_imag_cab, $strAlign);
?></tr></table>
</div>
<?php
/* Control de iframe */
...
<?php echo $this->nm_show_toolbarmenu('', $saida_apl, $mnu_pedidos_menuData, $path_imag_cab); ?><?php echo $this->nm_gera_degrade(1, $bg_line_degrade, $path_imag_cab); ?>  <tr>
<?php
}
else
{
...
?>
    <div id="Iframe_control" style='width:100%; height:100%; margin:0px; padding:0px;'>
      <iframe id="iframe_mnu_pedidos" name="mnu_pedidos_iframe" frameborder="0" class="scMenuIframe" style="width: 100%; height: 100%;"  src="<?php echo ($NM_scr_iframe != "" ? $NM_scr_iframe : "mnu_pedidos_pag_ini.php"); ?>"></iframe>
<?php
}
?></div></td>
  </tr>
...
/* Control de Target */
function mnu_pedidos_escreveMenu($arr_menu, $path_imag_cab = '', $strAlign = '')
{
    global $nm_data_fixa;
    $last      = '';
    $itemClass = ' topfirst';
...
function mnu_pedidos_target($str_target)
{
    global $mnu_pedidos_menuData;
    ...
    elseif ($mnu_pedidos_menuData['iframe'])
    {
        return 'mnu_pedidos_iframe';
    }
    else
    {
        return $str_target;
...
function nm_show_toolbarmenu($col_span, $saida_apl, $mnu_pedidos_menuData, $path_imag_cab)
{
   ...
   function nm_gera_menus(&$str_line_ret, $arr_menu_usu, $int_level, $nome_aplicacao)
   {
       ...
       foreach ($arr_menu_usu as $arr_item)
       {
           ...
           if ('' != $arr_item['link'])
           {
               ...
               {
                    $str_line['link'] = "mnu_pedidos_form_php.php?sc_item_menu=" . $arr_item['id'] . "&sc_apl_menu=" . $nome_apl . "&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . ""; 
               }
               else
               {
                    $str_line['link'] = "mnu_pedidos_form_php.php?sc_item_menu=" . $arr_item['id'] . "&sc_apl_menu=" . $nome_apl . "&sc_apl_link=" . urlencode($mnu_pedidos_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_usa_grupo'] . ""; 
           ...
           {
                $str_line['target'] = ('' != $arr_item['target'] && '' != $arr_item['link']) ?  $this->mnu_pedidos_target( $arr_item['target']) : "_self"; 
           }
   ...
   {
        $_SESSION['scriptcase']['mnu_pedidos']['sc_init'][$apl_menu] = 1;
        return  1;
   }
...
//
   function NM_gera_log_insert($orig="Scriptcase", $evento="", $texto="")
   {
       ...
       if (isset($_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date']) && !empty($_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date']))
       {
           $delim  = $_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date'];
           $delim1 = $_SESSION['scriptcase']['mnu_pedidos']['SC_sep_date1'];
       }
       ...
       if (in_array(strtolower($_SESSION['scriptcase']['glo_tpbanco']), $this->nm_bases_access))
       { 
           $comando = "INSERT INTO sis_log (inserted_date, username, application, creator, ip_user, `action`, description) VALUES ($dt, " . $this->Db->qstr($usr) . ", 'mnu_pedidos', '$orig', '" . $_SERVER['REMOTE_ADDR'] . "', '$evento', " . $this->Db->qstr($texto) . ")"; 
       } 
       elseif (in_array(strtolower($_SESSION['scriptcase']['glo_tpbanco']), $this->nm_bases_sqlite))
       { 
           $comando = "INSERT INTO sis_log (id, inserted_date, username, application, creator, ip_user, action, description) VALUES (NULL, $dt, " . $this->Db->qstr($usr) . ", 'mnu_pedidos', '$orig', '" . $_SERVER['REMOTE_ADDR'] . "', '$evento', " . $this->Db->qstr($texto) . ")"; 
       } 
       else
       { 
           $comando = "INSERT INTO sis_log (inserted_date, username, application, creator, ip_user, action, description) VALUES ($dt, " . $this->Db->qstr($usr) . ", 'mnu_pedidos', '$orig', '" . $_SERVER['REMOTE_ADDR'] . "', '$evento', " . $this->Db->qstr($texto) . ")"; 
...
if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
    ...
    elseif (is_file($root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_imag_temp'] . "/sc_apl_default_debi_doc_electronicos.txt")) {
        $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['mnu_pedidos']['glo_nm_path_imag_temp'] . "/sc_apl_default_debi_doc_electronicos.txt"));
    }
    if (isset($apl_def)) {
        if ($apl_def[0] != "mnu_pedidos") {
            $_SESSION['scriptcase']['sem_session'] = true;
            if (strtolower(substr($apl_def[0], 0 , 7)) == "http://" || strtolower(substr($apl_def[0], 0 , 8)) == "https://" || substr($apl_def[0], 0 , 2) == "..") {
                $_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir'] = $apl_def[0];
            }
            else {
                $_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir'] = $path_aplicacao . "/" . SC_dir_app_name($apl_def[0]) . "/index.php";
            }
            $Redir_tp = (isset($apl_def[1])) ? trim(strtoupper($apl_def[1])) : "";
            $_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['redir_tp'] = $Redir_tp;
        }
        if (isset($_COOKIE['sc_actual_lang_debi_doc_electronicos'])) {
            $_SESSION['scriptcase']['mnu_pedidos']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_debi_doc_electronicos'];
        }
    }
}
...
$contr_mnu_pedidos = new mnu_pedidos_class;
$contr_mnu_pedidos->mnu_pedidos_menu();
