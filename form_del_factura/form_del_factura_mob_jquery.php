...
function displayChange_field_detalle(row, status) {
    var fieldId;
	if ("on" == status && typeof $("#nmsc_iframe_liga_form_del_detalle_factura_mob")[0].contentWindow.scRecreateSelect2 === "function") {
		$("#nmsc_iframe_liga_form_del_detalle_factura_mob")[0].contentWindow.scRecreateSelect2();
	}
}
...
function scHidePage(pageNo) {
	$("#id_form_del_factura_mob_form" + pageNo).hide();
}
...
function scJQCalendarAdd(iSeqRow) {
  $("#id_sc_field_fac_fecha" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_del_factura_mob_validate_fac_fecha(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
...
?>
  $("#id_sc_field_fac_fecha_autorizacion" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_del_factura_mob_validate_fac_fecha_autorizacion(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_del_factura_mob')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
