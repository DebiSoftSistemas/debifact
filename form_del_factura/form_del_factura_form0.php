...
.css_read_off_fac_fecha_autorizacion button {
	background-color: transparent;
	border: 0;
	padding: 0
}
...
<script type="text/javascript" src="<?php echo $this->Ini->path_prod ?>/third/jquery_plugin/select2/js/select2.full.min.js"></script>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>form_del_factura/form_del_factura_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("form_del_factura_sajax_js.php");
?>
<script type="text/javascript">
...
var Nav_binicio_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['first']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['first'] : 'off'); ?>";
var Nav_bavanca_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['forward']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['forward'] : 'off'); ?>";
var Nav_bretorna_macro_disabled = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['back']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['back'] : 'off'); ?>";
var Nav_bfinal_macro_disabled   = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['last']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['last'] : 'off'); ?>";
...
function nav_desliga_img()
{
 ...
 function nm_field_disabled(Fields, Opt) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "ruc")
     {
        $('input[name="ruc"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="ruc"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="ruc"]').removeClass("scFormInputDisabled");
        }
     }
     ...
     if (F_name == "fac_cambio")
     {
        $('input[name="fac_cambio"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="fac_cambio"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="fac_cambio"]').removeClass("scFormInputDisabled");
        }
     }
  }
 } // nm_field_disabled
...
include_once('form_del_factura_jquery.php');
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['recarga'];
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
    $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['remove_border'] ? 'border-width: 0; ' : '';
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['link_info']['remove_margin']) {
        $remove_margin = 'margin: 0; ';
    }
    if ('' != $remove_margin && isset($str_iframe_body) && '' != $str_iframe_body) {
        $str_iframe_body = '';
    }
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['link_info']['remove_border']) {
...
<?php
 include_once("form_del_factura_js0.php");
?>
<script type="text/javascript"> 
...
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['insert_validation'] = md5(time() . rand(1, 99999));
?>
<input type="hidden" name="nmgp_ins_valid" value="<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['insert_validation']; ?>">
<?php
}
?>
...
<?php
$_SESSION['scriptcase']['error_span_title']['form_del_factura'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['form_del_factura'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
?>
<?php
if (($this->Embutida_form || !$this->Embutida_call || $this->Grid_editavel || $this->Embutida_multi || ($this->Embutida_call && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['embutida_liga_form_btn_nav'])) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] != "R")
{
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['salir']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['salir']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['salir']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['salir']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['salir'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label'][''];
        }
?>
<?php
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['nuevo']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['nuevo']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['nuevo']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['nuevo']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['nuevo'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['update']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['update']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['update']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['update']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['update'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['anular']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['anular']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['anular']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['anular']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['anular'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['enviar']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['enviar']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['enviar']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['enviar']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['enviar'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['group_group_1']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['group_group_1']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['group_group_1']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['group_group_1']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['group_group_1'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['ride']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['ride']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['ride']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['ride']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['ride'];
        }
?>
  <div class="scBtnGrpText scBtnGrpClick"  style="<?php echo $sCondStyle; ?>" id="gbl_sc_ride_top">
<?php echo nmButtonOutput($this->arr_buttons, "ride", "scBtnFn_ride()", "scBtnFn_ride()", "sc_ride_top", "", "" . $buttonMacroLabel . "", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "" . $buttonMacroDisabled . "", "", "group_1");?>
  </div>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['bauchert']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['bauchert']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['bauchert']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['bauchert']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['bauchert'];
        }
?>
  <div class="scBtnGrpText scBtnGrpClick"  style="<?php echo $sCondStyle; ?>" id="gbl_sc_bauchert_top">
<?php echo nmButtonOutput($this->arr_buttons, "bauchert", "scBtnFn_bauchert()", "scBtnFn_bauchert()", "sc_bauchert_top", "", "" . $buttonMacroLabel . "", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "" . $buttonMacroDisabled . "", "", "group_1");?>
  </div>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['retencion']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['retencion']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['retencion']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['retencion']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['retencion'];
        }
?>
...
<?php 
    if ($opcao_botoes != "novo") {
        $sCondStyle = ($this->nmgp_botoes['facturar_socio'] == "on") ? '' : 'display: none;';
?>
<?php
        $buttonMacroDisabled = '';
        $buttonMacroLabel = "";

        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['facturar_socio']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['facturar_socio']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['facturar_socio']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['facturar_socio']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['facturar_socio'];
        }
?>
<?php echo nmButtonOutput($this->arr_buttons, "facturar_socio", "scBtnFn_facturar_socio()", "scBtnFn_facturar_socio()", "sc_facturar_socio_top", "", "" . $buttonMacroLabel . "", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "" . $buttonMacroDisabled . "", "", "");?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['contabilizar']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['contabilizar']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['contabilizar']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['contabilizar']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['contabilizar'];
        }
?>
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['where_filter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['empty_filter'] = true;
       }
  }
?>
...
<?php
   ...
   if (!isset($this->nmgp_cmp_hidden['fac_factura_socio']))
   {
       $this->nmgp_cmp_hidden['fac_factura_socio'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['fac_correo_enviado']))
   {
       $this->nmgp_cmp_hidden['fac_correo_enviado'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['fac_asiento']))
   {
       $this->nmgp_cmp_hidden['fac_asiento'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['ambiente']))
   {
       $this->nmgp_cmp_hidden['ambiente'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['tipo_libretin']))
   {
       $this->nmgp_cmp_hidden['tipo_libretin'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['establecimiento']))
   {
       $this->nmgp_cmp_hidden['establecimiento'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['punto_emision']))
   {
       $this->nmgp_cmp_hidden['punto_emision'] = 'off';
   }
?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['fac_factura_socio']))
    {
        $this->nm_new_label['fac_factura_socio'] = "Fac Factura Socio";
    }
?>
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $fac_factura_socio = $this->fac_factura_socio;
   if (!isset($this->nmgp_cmp_hidden['fac_factura_socio']))
   {
       $this->nmgp_cmp_hidden['fac_factura_socio'] = 'off';
   }
   $sStyleHidden_fac_factura_socio = '';
   if (isset($this->nmgp_cmp_hidden['fac_factura_socio']) && $this->nmgp_cmp_hidden['fac_factura_socio'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['fac_factura_socio']);
       $sStyleHidden_fac_factura_socio = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_fac_factura_socio = 'display: none;';
   $sStyleReadInp_fac_factura_socio = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['fac_factura_socio']) && $this->nmgp_cmp_readonly['fac_factura_socio'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['fac_factura_socio']);
       $sStyleReadLab_fac_factura_socio = '';
       $sStyleReadInp_fac_factura_socio = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['fac_factura_socio']) && $this->nmgp_cmp_hidden['fac_factura_socio'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="fac_factura_socio" value="<?php echo $this->form_encode_input($fac_factura_socio) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_fac_factura_socio_line" id="hidden_field_data_fac_factura_socio" style="<?php echo $sStyleHidden_fac_factura_socio; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_factura_socio_line" style="vertical-align: top;padding: 0px"><input type="hidden" name="fac_factura_socio" value="<?php echo $this->form_encode_input($fac_factura_socio); ?>"><span id="id_ajax_label_fac_factura_socio"><?php echo nl2br($fac_factura_socio); ?></span>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_fac_factura_socio_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_fac_factura_socio_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_libretin']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_libretin'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_libretin']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_libretin'] = array(); 
}
   ...
   $old_value_fac_factura_socio = $this->fac_factura_socio;
   ...
   $old_value_fac_valor_entrega = $this->fac_valor_entrega;
   ...
   $unformatted_value_fac_factura_socio = $this->fac_factura_socio;
   ...
   $this->fac_valor_entrega = $old_value_fac_valor_entrega;
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_libretin'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_moneda']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_moneda'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_moneda']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_moneda'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_moneda'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php $sStyleHidden_fac_moneda_dumb = ('' == $sStyleHidden_fac_moneda) ? 'display: none' : ''; ?>
    <TD class="scFormDataOdd" id="hidden_field_data_fac_moneda_dumb" style="<?php echo $sStyleHidden_fac_moneda_dumb; ?>"></TD>
...
<?php $sStyleHidden_fac_tipo_comprobante_dumb = ('' == $sStyleHidden_fac_tipo_comprobante) ? 'display: none' : ''; ?>
    <TD class="scFormDataOdd" id="hidden_field_data_fac_tipo_comprobante_dumb" style="<?php echo $sStyleHidden_fac_tipo_comprobante_dumb; ?>"></TD>
   </tr>
<?php $sc_hidden_no = 1; ?>
</TABLE></div><!-- bloco_f -->
   </td>
...
<TABLE align="center" id="hidden_bloco_2" class="scFormTable<?php echo $this->classes_100perc_fields['table'] ?>" width="100%" style="height: 100%;"><?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
   if (!isset($this->nm_new_label['fac_socio_transportista']))
   {
       $this->nm_new_label['fac_socio_transportista'] = "" . $this->Ini->Nm_lang['lang_socio'] . "";
   }
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $fac_socio_transportista = $this->fac_socio_transportista;
   $sStyleHidden_fac_socio_transportista = '';
   if (isset($this->nmgp_cmp_hidden['fac_socio_transportista']) && $this->nmgp_cmp_hidden['fac_socio_transportista'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['fac_socio_transportista']);
       $sStyleHidden_fac_socio_transportista = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_fac_socio_transportista = 'display: none;';
   $sStyleReadInp_fac_socio_transportista = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['fac_socio_transportista']) && $this->nmgp_cmp_readonly['fac_socio_transportista'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['fac_socio_transportista']);
       $sStyleReadLab_fac_socio_transportista = '';
       $sStyleReadInp_fac_socio_transportista = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['fac_socio_transportista']) && $this->nmgp_cmp_hidden['fac_socio_transportista'] == 'off') { $sc_hidden_yes++; ?>
<input type=hidden name="fac_socio_transportista" value="<?php echo $this->form_encode_input($this->fac_socio_transportista) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_fac_socio_transportista_line" id="hidden_field_data_fac_socio_transportista" style="<?php echo $sStyleHidden_fac_socio_transportista; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_socio_transportista_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_fac_socio_transportista_label" style=""><span id="id_label_fac_socio_transportista"><?php echo $this->nm_new_label['fac_socio_transportista']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["fac_socio_transportista"]) &&  $this->nmgp_cmp_readonly["fac_socio_transportista"] == "on") { 
 
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_socio_transportista']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_socio_transportista'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_socio_transportista']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_socio_transportista'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   $nm_comando = "SELECT usu_usuario,usu_nombre FROM del_usuario_socio inner join del_usuario on  us_socio = usu_usuario where us_usuario='" . $_SESSION['Igtech_SesionLogin'] . "' ORDER BY us_socio";
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_socio_transportista'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
   ...
   while (!empty($todo[$x])) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          if (isset($this->Embutida_ronly) && $this->Embutida_ronly && isset($this->fac_socio_transportista_1))
          {
              foreach ($this->fac_socio_transportista_1 as $tmp_fac_socio_transportista)
              {
                  if (trim($tmp_fac_socio_transportista) === trim($cadaselect[1])) {$fac_socio_transportista_look .= $cadaselect[0] . '__SC_BREAK_LINE__';}
              }
          }
          elseif (isset($cadaselect[1]) && is_string($this->fac_socio_transportista) && trim($this->fac_socio_transportista) === trim($cadaselect[1])) {$fac_socio_transportista_look .= $cadaselect[0];} 
          $x++; 
   }

?>
<input type="hidden" name="fac_socio_transportista" value="<?php echo $this->form_encode_input($fac_socio_transportista) . "\">" . $fac_socio_transportista_look . ""; ?>
<?php } else { ?>
<?php
   $todo = $this->Form_lookup_fac_socio_transportista();
   $x = 0 ; 
   $fac_socio_transportista_look = ""; 
   while (!empty($todo[$x])) 
   {
          ...
          if (empty($fac_socio_transportista_look))
          {
              $fac_socio_transportista_look = $this->fac_socio_transportista;
          }
   $x = 0; 
   echo "<span id=\"id_read_on_fac_socio_transportista\" class=\"css_fac_socio_transportista_line\" style=\"" .  $sStyleReadLab_fac_socio_transportista . "\">" . $this->form_format_readonly("fac_socio_transportista", $this->form_encode_input($fac_socio_transportista_look)) . "</span><span id=\"id_read_off_fac_socio_transportista\" class=\"css_read_off_fac_socio_transportista" . $this->classes_100perc_fields['span_input'] . "\" style=\"white-space: nowrap; " . $sStyleReadInp_fac_socio_transportista . "\">";
   echo " <span id=\"idAjaxSelect_fac_socio_transportista\" class=\"" . $this->classes_100perc_fields['span_select'] . "\"><select class=\"sc-js-input scFormObjectOdd css_fac_socio_transportista_obj" . $this->classes_100perc_fields['input'] . "\" style=\"\" id=\"id_sc_field_fac_socio_transportista\" name=\"fac_socio_transportista\" size=\"1\" alt=\"{type: 'select', enterTab: false}\">" ; 
   echo "\r" ; 
   $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_socio_transportista'][] = ''; 
   ...
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          ...
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          echo "  <option value=\"$cadaselect[1]\"" ; 
          if (trim($this->fac_socio_transportista) === trim($cadaselect[1])) 
          {
              echo " selected" ; 
          }
          if (strtoupper($cadaselect[2]) == "S") 
          {
              if (empty($this->fac_socio_transportista)) 
              {
                  echo " selected" ;
              } 
           } 
...
?> 
<?php  }?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_fac_socio_transportista_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_fac_socio_transportista_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>

   <?php
   if (!isset($this->nm_new_label['fac_transporte_usuario']))
   {
       $this->nm_new_label['fac_transporte_usuario'] = "" . $this->Ini->Nm_lang['lang_vehiculos'] . "";
   }
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $fac_transporte_usuario = $this->fac_transporte_usuario;
   $sStyleHidden_fac_transporte_usuario = '';
   if (isset($this->nmgp_cmp_hidden['fac_transporte_usuario']) && $this->nmgp_cmp_hidden['fac_transporte_usuario'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['fac_transporte_usuario']);
       $sStyleHidden_fac_transporte_usuario = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_fac_transporte_usuario = 'display: none;';
   $sStyleReadInp_fac_transporte_usuario = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['fac_transporte_usuario']) && $this->nmgp_cmp_readonly['fac_transporte_usuario'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['fac_transporte_usuario']);
       $sStyleReadLab_fac_transporte_usuario = '';
       $sStyleReadInp_fac_transporte_usuario = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['fac_transporte_usuario']) && $this->nmgp_cmp_hidden['fac_transporte_usuario'] == 'off') { $sc_hidden_yes++; ?>
<input type=hidden name="fac_transporte_usuario" value="<?php echo $this->form_encode_input($this->fac_transporte_usuario) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_fac_transporte_usuario_line" id="hidden_field_data_fac_transporte_usuario" style="<?php echo $sStyleHidden_fac_transporte_usuario; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_transporte_usuario_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_fac_transporte_usuario_label" style=""><span id="id_label_fac_transporte_usuario"><?php echo $this->nm_new_label['fac_transporte_usuario']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["fac_transporte_usuario"]) &&  $this->nmgp_cmp_readonly["fac_transporte_usuario"] == "on") { 
 
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_transporte_usuario']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_transporte_usuario'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_transporte_usuario']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_transporte_usuario'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   $nm_comando = "SELECT tu_id, tu_placa  FROM del_transporte_usuario  where tu_usuario='$this->fac_socio_transportista' ORDER BY tu_placa";
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_transporte_usuario'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
   ...
   while (!empty($todo[$x])) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          if (isset($this->Embutida_ronly) && $this->Embutida_ronly && isset($this->fac_transporte_usuario_1))
          {
              foreach ($this->fac_transporte_usuario_1 as $tmp_fac_transporte_usuario)
              {
                  if (trim($tmp_fac_transporte_usuario) === trim($cadaselect[1])) {$fac_transporte_usuario_look .= $cadaselect[0] . '__SC_BREAK_LINE__';}
              }
          }
          elseif (isset($cadaselect[1]) && is_string($this->fac_transporte_usuario) && trim($this->fac_transporte_usuario) === trim($cadaselect[1])) {$fac_transporte_usuario_look .= $cadaselect[0];} 
          $x++; 
   }

?>
<input type="hidden" name="fac_transporte_usuario" value="<?php echo $this->form_encode_input($fac_transporte_usuario) . "\">" . $fac_transporte_usuario_look . ""; ?>
<?php } else { ?>
<?php
   $todo = $this->Form_lookup_fac_transporte_usuario();
   $x = 0 ; 
   $fac_transporte_usuario_look = ""; 
   while (!empty($todo[$x])) 
   {
          ...
          if (empty($fac_transporte_usuario_look))
          {
              $fac_transporte_usuario_look = $this->fac_transporte_usuario;
          }
   $x = 0; 
   echo "<span id=\"id_read_on_fac_transporte_usuario\" class=\"css_fac_transporte_usuario_line\" style=\"" .  $sStyleReadLab_fac_transporte_usuario . "\">" . $this->form_format_readonly("fac_transporte_usuario", $this->form_encode_input($fac_transporte_usuario_look)) . "</span><span id=\"id_read_off_fac_transporte_usuario\" class=\"css_read_off_fac_transporte_usuario" . $this->classes_100perc_fields['span_input'] . "\" style=\"white-space: nowrap; " . $sStyleReadInp_fac_transporte_usuario . "\">";
   echo " <span id=\"idAjaxSelect_fac_transporte_usuario\" class=\"" . $this->classes_100perc_fields['span_select'] . "\"><select class=\"sc-js-input scFormObjectOdd css_fac_transporte_usuario_obj" . $this->classes_100perc_fields['input'] . "\" style=\"\" id=\"id_sc_field_fac_transporte_usuario\" name=\"fac_transporte_usuario\" size=\"1\" alt=\"{type: 'select', enterTab: false}\">" ; 
   echo "\r" ; 
   $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_transporte_usuario'][] = ''; 
   echo "  <option value=\"\">" . str_replace("<", "&lt;","" . $this->Ini->Nm_lang['lang_seleccione'] . "") . "</option>" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          echo "  <option value=\"$cadaselect[1]\"" ; 
          if (trim($this->fac_transporte_usuario) === trim($cadaselect[1])) 
          {
              echo " selected" ; 
          }
          if (strtoupper($cadaselect[2]) == "S") 
          {
              if (empty($this->fac_transporte_usuario)) 
              {
                  echo " selected" ;
              } 
           } 
...
?> 
<?php  }?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_fac_transporte_usuario_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_fac_transporte_usuario_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php } 
?> 
<?php $sStyleHidden_fac_socio_transportista_dumb = ('' == $sStyleHidden_fac_socio_transportista) ? 'display: none' : ''; ?>
    <TD class="scFormDataOdd" id="hidden_field_data_fac_socio_transportista_dumb" style="<?php echo $sStyleHidden_fac_socio_transportista_dumb; ?>"></TD>
<?php $sStyleHidden_fac_transporte_usuario_dumb = ('' == $sStyleHidden_fac_transporte_usuario) ? 'display: none' : ''; ?>
    <TD class="scFormDataOdd" id="hidden_field_data_fac_transporte_usuario_dumb" style="<?php echo $sStyleHidden_fac_transporte_usuario_dumb; ?>"></TD>
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
      $sc_hidden_yes = 0; $sc_hidden_no = 0; ?>


   <?php
    if (!isset($this->nm_new_label['fac_correo_enviado']))
    {
        $this->nm_new_label['fac_correo_enviado'] = "Fac Correo Enviado";
    }
?>
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $fac_correo_enviado = $this->fac_correo_enviado;
   if (!isset($this->nmgp_cmp_hidden['fac_correo_enviado']))
   {
       $this->nmgp_cmp_hidden['fac_correo_enviado'] = 'off';
   }
   $sStyleHidden_fac_correo_enviado = '';
   if (isset($this->nmgp_cmp_hidden['fac_correo_enviado']) && $this->nmgp_cmp_hidden['fac_correo_enviado'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['fac_correo_enviado']);
       $sStyleHidden_fac_correo_enviado = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_fac_correo_enviado = 'display: none;';
   $sStyleReadInp_fac_correo_enviado = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['fac_correo_enviado']) && $this->nmgp_cmp_readonly['fac_correo_enviado'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['fac_correo_enviado']);
       $sStyleReadLab_fac_correo_enviado = '';
       $sStyleReadInp_fac_correo_enviado = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['fac_correo_enviado']) && $this->nmgp_cmp_hidden['fac_correo_enviado'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="fac_correo_enviado" value="<?php echo $this->form_encode_input($fac_correo_enviado) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_fac_correo_enviado_line" id="hidden_field_data_fac_correo_enviado" style="<?php echo $sStyleHidden_fac_correo_enviado; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_correo_enviado_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_fac_correo_enviado_label" style=""><span id="id_label_fac_correo_enviado"><?php echo $this->nm_new_label['fac_correo_enviado']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["fac_correo_enviado"]) &&  $this->nmgp_cmp_readonly["fac_correo_enviado"] == "on") { 

 ?>
<input type="hidden" name="fac_correo_enviado" value="<?php echo $this->form_encode_input($fac_correo_enviado) . "\">" . $fac_correo_enviado . ""; ?>
<?php } else { ?>
<span id="id_read_on_fac_correo_enviado" class="sc-ui-readonly-fac_correo_enviado css_fac_correo_enviado_line" style="<?php echo $sStyleReadLab_fac_correo_enviado; ?>"><?php echo $this->form_format_readonly("fac_correo_enviado", $this->form_encode_input($this->fac_correo_enviado)); ?></span><span id="id_read_off_fac_correo_enviado" class="css_read_off_fac_correo_enviado<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_fac_correo_enviado; ?>">
 <input class="sc-js-input scFormObjectOdd css_fac_correo_enviado_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_fac_correo_enviado" type=text name="fac_correo_enviado" value="<?php echo $this->form_encode_input($fac_correo_enviado) ?>"
...
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_fac_correo_enviado_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_fac_correo_enviado_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>

   <?php
    if (!isset($this->nm_new_label['fac_asiento']))
    {
        $this->nm_new_label['fac_asiento'] = "Fac Asiento";
    }
?>
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $fac_asiento = $this->fac_asiento;
   if (!isset($this->nmgp_cmp_hidden['fac_asiento']))
   {
       $this->nmgp_cmp_hidden['fac_asiento'] = 'off';
   }
   $sStyleHidden_fac_asiento = '';
   if (isset($this->nmgp_cmp_hidden['fac_asiento']) && $this->nmgp_cmp_hidden['fac_asiento'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['fac_asiento']);
       $sStyleHidden_fac_asiento = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_fac_asiento = 'display: none;';
   $sStyleReadInp_fac_asiento = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['fac_asiento']) && $this->nmgp_cmp_readonly['fac_asiento'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['fac_asiento']);
       $sStyleReadLab_fac_asiento = '';
       $sStyleReadInp_fac_asiento = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['fac_asiento']) && $this->nmgp_cmp_hidden['fac_asiento'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="fac_asiento" value="<?php echo $this->form_encode_input($fac_asiento) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_fac_asiento_line" id="hidden_field_data_fac_asiento" style="<?php echo $sStyleHidden_fac_asiento; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_asiento_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_fac_asiento_label" style=""><span id="id_label_fac_asiento"><?php echo $this->nm_new_label['fac_asiento']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["fac_asiento"]) &&  $this->nmgp_cmp_readonly["fac_asiento"] == "on") { 

 ?>
<input type="hidden" name="fac_asiento" value="<?php echo $this->form_encode_input($fac_asiento) . "\">" . $fac_asiento . ""; ?>
<?php } else { ?>
<span id="id_read_on_fac_asiento" class="sc-ui-readonly-fac_asiento css_fac_asiento_line" style="<?php echo $sStyleReadLab_fac_asiento; ?>"><?php echo $this->form_format_readonly("fac_asiento", $this->form_encode_input($this->fac_asiento)); ?></span><span id="id_read_off_fac_asiento" class="css_read_off_fac_asiento<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_fac_asiento; ?>">
 <input class="sc-js-input scFormObjectOdd css_fac_asiento_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_fac_asiento" type=text name="fac_asiento" value="<?php echo $this->form_encode_input($fac_asiento) ?>"
...
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_fac_asiento_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_fac_asiento_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php $sStyleHidden_fac_correo_enviado_dumb = ('' == $sStyleHidden_fac_correo_enviado) ? 'display: none' : ''; ?>
    <TD class="scFormDataOdd" id="hidden_field_data_fac_correo_enviado_dumb" style="<?php echo $sStyleHidden_fac_correo_enviado_dumb; ?>"></TD>
<?php $sStyleHidden_fac_asiento_dumb = ('' == $sStyleHidden_fac_asiento) ? 'display: none' : ''; ?>
    <TD class="scFormDataOdd" id="hidden_field_data_fac_asiento_dumb" style="<?php echo $sStyleHidden_fac_asiento_dumb; ?>"></TD>
   </tr>
<?php $sc_hidden_no = 1; ?>
</TABLE></div><!-- bloco_f -->
   </td>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormLabelOdd scUiLabelWidthFix css_fac_cliente_label" id="hidden_field_label_fac_cliente" style="<?php echo $sStyleHidden_fac_cliente; ?>"><span id="id_label_fac_cliente"><?php echo $this->nm_new_label['fac_cliente']; ?></span><?php if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['php_cmp_required']['fac_cliente']) || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['php_cmp_required']['fac_cliente'] == "on") { ?> <span class="scFormRequiredOdd">*</span> <?php }?></TD>
    <TD class="scFormDataOdd css_fac_cliente_line" id="hidden_field_data_fac_cliente" style="<?php echo $sStyleHidden_fac_cliente; ?>vertical-align: top;"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_cliente_line" style="vertical-align: top;padding: 0px"><input type="hidden" name="fac_cliente" value="<?php echo $this->form_encode_input($fac_cliente); ?>"><span id="id_ajax_label_fac_cliente"><?php echo nl2br($fac_cliente); ?></span>
<?php
   $Sc_iframe_master = ($this->Embutida_call) ? 'nmgp_iframe_ret*scinnmsc_iframe_liga_form_del_factura*scout' : '';
   if (isset($this->Ini->sc_lig_md5["buscar_cliente"]) && $this->Ini->sc_lig_md5["buscar_cliente"] == "S") {
       $Parms_Lig  = "nmgp_url_saida*scin*scoutnmgp_parms_ret*scinF1,fac_cliente,cl_id*scoutnm_evt_ret_busca*scinsc_form_del_factura_fac_cliente_onchange(this)*scoutnmgp_perm_edit*scinN*scout" . $Sc_iframe_master;
       $Md5_Lig    = "@SC_par@" . $this->form_encode_input($this->Ini->sc_page) . "@SC_par@form_del_factura@SC_par@" . md5($Parms_Lig);
       $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lig_Md5'][md5($Parms_Lig)] = $Parms_Lig;
   } else {
       $Md5_Lig  = "nmgp_url_saida*scin*scoutnmgp_parms_ret*scinF1,fac_cliente,cl_id*scoutnm_evt_ret_busca*scinsc_form_del_factura_fac_cliente_onchange(this)*scoutnmgp_perm_edit*scinN*scout" . $Sc_iframe_master;
   }
?>

<?php if (!$this->Ini->Export_img_zip) { ?><?php echo nmButtonOutput($this->arr_buttons, "bform_captura", "nm_submit_cap('" . $this->Ini->link_buscar_cliente_cons_psq. "', '" . $Md5_Lig . "')", "nm_submit_cap('" . $this->Ini->link_buscar_cliente_cons_psq. "', '" . $Md5_Lig . "')", "cap_fac_cliente", "", "", "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
<?php } ?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_ruc_line" id="hidden_field_data_ruc" style="<?php echo $sStyleHidden_ruc; ?>vertical-align: top;"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_ruc_line" style="vertical-align: top;padding: 0px">
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormLabelOdd scUiLabelWidthFix css_fac_aplica_propina_label" id="hidden_field_label_fac_aplica_propina" style="<?php echo $sStyleHidden_fac_aplica_propina; ?>"><span id="id_label_fac_aplica_propina"><?php echo $this->nm_new_label['fac_aplica_propina']; ?></span></TD>
    <TD class="scFormDataOdd css_fac_aplica_propina_line" id="hidden_field_data_fac_aplica_propina" style="<?php echo $sStyleHidden_fac_aplica_propina; ?>vertical-align: top;"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_aplica_propina_line" style="vertical-align: top;padding: 0px">
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_aplica_propina']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_aplica_propina'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_aplica_propina']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_aplica_propina'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_aplica_propina'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormLabelOdd scUiLabelWidthFix css_fac_diagnostico_label" id="hidden_field_label_fac_diagnostico" style="<?php echo $sStyleHidden_fac_diagnostico; ?>"><span id="id_label_fac_diagnostico"><?php echo $this->nm_new_label['fac_diagnostico']; ?></span></TD>
    <TD class="scFormDataOdd css_fac_diagnostico_line" id="hidden_field_data_fac_diagnostico" style="<?php echo $sStyleHidden_fac_diagnostico; ?>vertical-align: top;"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_diagnostico_line" style="vertical-align: top;padding: 0px">
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_diagnostico']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_diagnostico'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_diagnostico']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_diagnostico'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_diagnostico'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php
   ...
   echo "\r" ; 
   $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lookup_fac_diagnostico'][] = ''; 
   echo "  <option value=\"\">" . str_replace("<", "&lt;","" . $this->Ini->Nm_lang['lang_seleccione'] . "") . "</option>" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
   ...
   if (isset($this->Ini->sc_lig_md5["form_del_diagnosticos"]) && $this->Ini->sc_lig_md5["form_del_diagnosticos"] == "S") {
       $Parms_Lig  = "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutnm_evt_ret_edit*scindo_ajax_form_del_factura_lkpedt_refresh_fac_diagnostico*scoutnmgp_url_saida*scinmodal*scoutnmgp_outra_jan*scintrue*scoutsc_redir_atualiz*scinok*scout";
       $Md5_Lig    = "@SC_par@" . $this->form_encode_input($this->Ini->sc_page) . "@SC_par@form_del_factura@SC_par@" . md5($Parms_Lig);
       $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['Lig_Md5'][md5($Parms_Lig)] = $Parms_Lig;
   } else {
       $Md5_Lig  = "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutnm_evt_ret_edit*scindo_ajax_form_del_factura_lkpedt_refresh_fac_diagnostico*scoutnmgp_url_saida*scinmodal*scoutnmgp_outra_jan*scintrue*scoutsc_redir_atualiz*scinok*scout";
   }
 ?><?php echo nmButtonOutput($this->arr_buttons, "bform_lookuplink", "", "", "fldedt_fac_diagnostico", "", "", "", "", "", "", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->link_form_del_diagnosticos_edit . "?script_case_init=" . $this->Ini->sc_page . "&nmgp_parms=" . $Md5_Lig . "&SC_lig_apl_orig=form_del_factura&KeepThis=true&TB_iframe=true&height=300&width=600&modal=true", "");?>
<?php    echo "</span>";
?> 
<?php  }?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormLabelOdd scUiLabelWidthFix css_fac_comentario_label" id="hidden_field_label_fac_comentario" style="<?php echo $sStyleHidden_fac_comentario; ?>"><span id="id_label_fac_comentario"><?php echo $this->nm_new_label['fac_comentario']; ?></span></TD>
    <TD class="scFormDataOdd css_fac_comentario_line" id="hidden_field_data_fac_comentario" style="<?php echo $sStyleHidden_fac_comentario; ?>vertical-align: top;"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_comentario_line" style="vertical-align: top;padding: 0px">
<?php
...
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["fac_comentario"]) &&  $this->nmgp_cmp_readonly["fac_comentario"] == "on") { 
 ...
 ?>
<input type="hidden" name="fac_comentario" value="<?php echo $this->form_encode_input($fac_comentario) . "\">" . $fac_comentario_val . ""; ?>
<?php } else { ?>
<span id="id_read_on_fac_comentario" class="sc-ui-readonly-fac_comentario css_fac_comentario_line" style="<?php echo $sStyleReadLab_fac_comentario; ?>"><?php echo $this->form_format_readonly("fac_comentario", $this->form_encode_input($fac_comentario_val)); ?></span><span id="id_read_off_fac_comentario" class="css_read_off_fac_comentario<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_fac_comentario; ?>">
 <textarea class="sc-js-input scFormObjectOdd css_fac_comentario_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="white-space: pre-wrap;resize: vertical;" name="fac_comentario" id="id_sc_field_fac_comentario" rows="2" cols="50"
...
<?php
 if (isset($_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_detalle'] ]) && '' != $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_detalle'] ]) {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] = $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_detalle'] ];
 }
 else {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] = $this->Ini->sc_page;
 }
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_proc']  = false;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_form']  = true;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_call']  = true;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_multi'] = false;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_form_insert'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_form_update'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_form_delete'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_form_btn_nav'] = 'off';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_grid_edit'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_grid_edit_link'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_qtd_reg'] = '10';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_liga_tp_pag'] = 'total';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['embutida_parms'] = "var_factura*scin" . $this->nmgp_dados_form['fac_numero'] . "*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout";
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['foreign_key']['df_factura'] = $this->nmgp_dados_form['fac_numero'];
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['where_filter'] = "df_factura = " . $this->nmgp_dados_form['fac_numero'] . "";
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['where_detal']  = "df_factura = " . $this->nmgp_dados_form['fac_numero'] . "";
 if ($_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_factura']['total'] < 0)
 {
     $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init'] ]['form_del_detalle_factura']['where_filter'] = "1 <> 1";
 }
 $sDetailSrc = ('novo' == $this->nmgp_opcao) ? 'form_del_factura_empty.htm' : $this->Ini->link_form_del_detalle_factura_edit . '?script_case_init=' . $this->form_encode_input($this->Ini->sc_page) . '&script_case_detail=Y&sc_ifr_height=300';
if (isset($this->Ini->sc_lig_target['C_@scinf_detalle']) && 'nmsc_iframe_liga_form_del_detalle_factura' != $this->Ini->sc_lig_target['C_@scinf_detalle'])
{
    if ('novo' != $this->nmgp_opcao)
    {
        $sDetailSrc .= '&under_dashboard=1&dashboard_app=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] . '&own_widget=' . $this->Ini->sc_lig_target['C_@scinf_detalle'] . '&parent_widget=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['own_widget'];
        $sDetailSrc  = $this->addUrlParam($sDetailSrc, 'script_case_init', $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_detalle_factura_script_case_init']);
    }
?>
<script type="text/javascript">
$(function() {
...
{
?>
<iframe border="0" id="nmsc_iframe_liga_form_del_detalle_factura"  marginWidth="0" marginHeight="0" frameborder="0" valign="top" height="300" width="100%" name="nmsc_iframe_liga_form_del_detalle_factura"  scrolling="auto" src="<?php echo $sDetailSrc; ?>"></iframe>
<?php
}
?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['fac_valor_entrega']))
    {
        $this->nm_new_label['fac_valor_entrega'] = "" . $this->Ini->Nm_lang['lang_pago'] . "";
    }
?>
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $fac_valor_entrega = $this->fac_valor_entrega;
   $sStyleHidden_fac_valor_entrega = '';
   if (isset($this->nmgp_cmp_hidden['fac_valor_entrega']) && $this->nmgp_cmp_hidden['fac_valor_entrega'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['fac_valor_entrega']);
       $sStyleHidden_fac_valor_entrega = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_fac_valor_entrega = 'display: none;';
   $sStyleReadInp_fac_valor_entrega = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['fac_valor_entrega']) && $this->nmgp_cmp_readonly['fac_valor_entrega'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['fac_valor_entrega']);
       $sStyleReadLab_fac_valor_entrega = '';
       $sStyleReadInp_fac_valor_entrega = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['fac_valor_entrega']) && $this->nmgp_cmp_hidden['fac_valor_entrega'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="fac_valor_entrega" value="<?php echo $this->form_encode_input($fac_valor_entrega) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormLabelOdd scUiLabelWidthFix css_fac_valor_entrega_label" id="hidden_field_label_fac_valor_entrega" style="<?php echo $sStyleHidden_fac_valor_entrega; ?>"><span id="id_label_fac_valor_entrega"><?php echo $this->nm_new_label['fac_valor_entrega']; ?></span></TD>
    <TD class="scFormDataOdd css_fac_valor_entrega_line" id="hidden_field_data_fac_valor_entrega" style="<?php echo $sStyleHidden_fac_valor_entrega; ?>vertical-align: top;"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_valor_entrega_line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["fac_valor_entrega"]) &&  $this->nmgp_cmp_readonly["fac_valor_entrega"] == "on") { 

 ?>
<input type="hidden" name="fac_valor_entrega" value="<?php echo $this->form_encode_input($fac_valor_entrega) . "\">" . $fac_valor_entrega . ""; ?>
<?php } else { ?>
<span id="id_read_on_fac_valor_entrega" class="sc-ui-readonly-fac_valor_entrega css_fac_valor_entrega_line" style="<?php echo $sStyleReadLab_fac_valor_entrega; ?>"><?php echo $this->form_format_readonly("fac_valor_entrega", $this->form_encode_input($this->fac_valor_entrega)); ?></span><span id="id_read_off_fac_valor_entrega" class="css_read_off_fac_valor_entrega<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_fac_valor_entrega; ?>">
 <input class="sc-js-input scFormObjectOdd css_fac_valor_entrega_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_fac_valor_entrega" type=text name="fac_valor_entrega" value="<?php echo $this->form_encode_input($fac_valor_entrega) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=10"; } ?> alt="{datatype: 'decimal', maxLength: 20, precision: 2, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['fac_valor_entrega']['symbol_dec']); ?>', thousandsSep: '', thousandsFormat: <?php echo $this->field_config['fac_valor_entrega']['symbol_fmt']; ?>, manualDecimals: true, allowNegative: true, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['fac_valor_entrega']['format_neg'] ? "'suffix'" : "'prefix'") ?>, enterTab: false, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_fac_valor_entrega_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_fac_valor_entrega_text"></span></td></tr></table></td></tr></table></TD>
   <?php }?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['fac_cambio']))
    {
...
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $fac_cambio = $this->fac_cambio;
   $sStyleHidden_fac_cambio = '';
   if (isset($this->nmgp_cmp_hidden['fac_cambio']) && $this->nmgp_cmp_hidden['fac_cambio'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['fac_cambio']);
       $sStyleHidden_fac_cambio = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_fac_cambio = 'display: none;';
   $sStyleReadInp_fac_cambio = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['fac_cambio']) && $this->nmgp_cmp_readonly['fac_cambio'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['fac_cambio']);
       $sStyleReadLab_fac_cambio = '';
       $sStyleReadInp_fac_cambio = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['fac_cambio']) && $this->nmgp_cmp_hidden['fac_cambio'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="fac_cambio" value="<?php echo $this->form_encode_input($fac_cambio) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormLabelOdd scUiLabelWidthFix css_fac_cambio_label" id="hidden_field_label_fac_cambio" style="<?php echo $sStyleHidden_fac_cambio; ?>"><span id="id_label_fac_cambio"><?php echo $this->nm_new_label['fac_cambio']; ?></span></TD>
    <TD class="scFormDataOdd css_fac_cambio_line" id="hidden_field_data_fac_cambio" style="<?php echo $sStyleHidden_fac_cambio; ?>vertical-align: top;"><table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_fac_cambio_line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["fac_cambio"]) &&  $this->nmgp_cmp_readonly["fac_cambio"] == "on") { 

 ?>
<input type="hidden" name="fac_cambio" value="<?php echo $this->form_encode_input($fac_cambio) . "\">" . $fac_cambio . ""; ?>
<?php } else { ?>
<span id="id_read_on_fac_cambio" class="sc-ui-readonly-fac_cambio css_fac_cambio_line" style="<?php echo $sStyleReadLab_fac_cambio; ?>"><?php echo $this->form_format_readonly("fac_cambio", $this->form_encode_input($this->fac_cambio)); ?></span><span id="id_read_off_fac_cambio" class="css_read_off_fac_cambio<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_fac_cambio; ?>">
 <input class="sc-js-input scFormObjectOdd css_fac_cambio_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_fac_cambio" type=text name="fac_cambio" value="<?php echo $this->form_encode_input($fac_cambio) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=10"; } ?> alt="{datatype: 'decimal', maxLength: 20, precision: 2, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['fac_cambio']['symbol_dec']); ?>', thousandsSep: '', thousandsFormat: <?php echo $this->field_config['fac_cambio']['symbol_fmt']; ?>, manualDecimals: false, allowNegative: true, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['fac_cambio']['format_neg'] ? "'suffix'" : "'prefix'") ?>, enterTab: false, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_fac_cambio_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_fac_cambio_text"></span></td></tr></table></td></tr></table></TD>
   <?php }?>
...
<?php
 if (isset($_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_forma_pago'] ]) && '' != $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_forma_pago'] ]) {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] = $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_forma_pago'] ];
 }
 else {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] = $this->Ini->sc_page;
 }
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_proc']  = false;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_form']  = true;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_call']  = true;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_multi'] = false;
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_form_insert'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_form_update'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_form_delete'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_form_btn_nav'] = 'off';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_grid_edit'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_grid_edit_link'] = 'on';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_qtd_reg'] = '10';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_liga_tp_pag'] = 'total';
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['embutida_parms'] = "var_factura*scin" . $this->nmgp_dados_form['fac_numero'] . "*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout";
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['foreign_key']['fp_factura'] = $this->nmgp_dados_form['fac_numero'];
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['where_filter'] = "fp_factura = " . $this->nmgp_dados_form['fac_numero'] . "";
 $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['where_detal']  = "fp_factura = " . $this->nmgp_dados_form['fac_numero'] . "";
 if ($_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_factura']['total'] < 0)
 {
     $_SESSION['sc_session'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init'] ]['form_del_forma_pago_factura']['where_filter'] = "1 <> 1";
 }
 $sDetailSrc = ('novo' == $this->nmgp_opcao) ? 'form_del_factura_empty.htm' : $this->Ini->link_form_del_forma_pago_factura_edit . '?script_case_init=' . $this->form_encode_input($this->Ini->sc_page) . '&script_case_detail=Y&sc_ifr_height=200';
if (isset($this->Ini->sc_lig_target['C_@scinf_forma_pago']) && 'nmsc_iframe_liga_form_del_forma_pago_factura' != $this->Ini->sc_lig_target['C_@scinf_forma_pago'])
{
    if ('novo' != $this->nmgp_opcao)
    {
        $sDetailSrc .= '&under_dashboard=1&dashboard_app=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['dashboard_app'] . '&own_widget=' . $this->Ini->sc_lig_target['C_@scinf_forma_pago'] . '&parent_widget=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['own_widget'];
        $sDetailSrc  = $this->addUrlParam($sDetailSrc, 'script_case_init', $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['form_del_forma_pago_factura_script_case_init']);
    }
?>
<script type="text/javascript">
$(function() {
...
{
?>
<iframe border="0" id="nmsc_iframe_liga_form_del_forma_pago_factura"  marginWidth="0" marginHeight="0" frameborder="0" valign="top" height="200" width="100%" name="nmsc_iframe_liga_form_del_forma_pago_factura"  scrolling="auto" src="<?php echo $sDetailSrc; ?>"></iframe>
<?php
}
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['help']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['help']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['help']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['help']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['help'];
        }
?>
...
<?php
    ...
    if ((!$this->Embutida_call || $this->form_3versions_single) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && $nm_apl_dependente != 1 && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] != "R" && !$this->aba_iframe && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
        $buttonMacroDisabled = 'sc-unique-btn-4';
        $buttonMacroLabel = "";

        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['exit']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_disabled']['exit']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['exit']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['exit']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['btn_label']['exit'];
        }
?>
...
<?php
    ...
    if ((!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (!isset($_SESSION['scriptcase']['nm_sc_retorno']) || empty($_SESSION['scriptcase']['nm_sc_retorno']) || $nm_apl_dependente == 1 || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] == "R" || $this->aba_iframe || $this->nmgp_botoes['exit'] != "on") && ($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['run_iframe'] != "F" && $this->nmgp_botoes['exit'] == "on") && ($nm_apl_dependente == 1 && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['masterValue']))
{
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['under_dashboard']) {
?>
var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['parent_widget']; ?>']");
if (dbParentFrame && dbParentFrame[0] && dbParentFrame[0].contentWindow.scAjaxDetailValue)
{
<?php
        foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['masterValue'] as $cmp_master => $val_master)
        {
?>
    dbParentFrame[0].contentWindow.scAjaxDetailValue('<?php echo $cmp_master ?>', '<?php echo $val_master ?>');
<?php
        }
        unset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['masterValue']);
?>
}
<?php
    }
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("form_del_factura");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("form_del_factura");
 parent.scAjaxDetailHeight("form_del_factura", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
{
<?php
if ($this->lig_edit_lookup && isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['sc_modal'])
{
?>
...
<script type="text/javascript">
	...
	function scBtnFn_bauchert() {
		if ($("#sc_bauchert_top").length && $("#sc_bauchert_top").is(":visible")) {
		    if ($("#sc_bauchert_top").hasClass("disabled")) {
		        return;
		    }
			sc_btn_bauchert()
			 return;
	...
	function scBtnFn_facturar_socio() {
		if ($("#sc_facturar_socio_top").length && $("#sc_facturar_socio_top").is(":visible")) {
		    if ($("#sc_facturar_socio_top").hasClass("disabled")) {
		        return;
		    }
			sc_btn_facturar_socio()
			 return;
		}
	}
...
?>
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
