<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="ret_numero" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['ret_numero']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_retencion_dos']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_retencion_dos']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_retencion_dos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
    do_ajax_form_del_retencion_dos_event_scajaxbutton_enviar_onclick();
}
...
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nueva_factura; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin072*scout"; ?>', '', '_self', '0', '0', 'blank_nueva_factura');
}
var var_btn_ride_var_tipo_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['ret_tipo_comprobante']); ?>";
var var_btn_ride_var_id_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['ret_numero']); ?>";
...
{
    do_ajax_form_del_retencion_dos_event_scajaxbutton_anular_onclick();
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_retencion_dos']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_retencion_dos_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_retencion_dos_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
?>
    ...
    {
        do_ajax_form_del_retencion_dos_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_del_retencion_dos_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
...
{
   nm_field_disabled("secuencial=disabled;tipo_libretin=disabled;ambiente=disabled;ret_ruc_cliente=disabled;ret_nombre_cliente=disabled;ret_direccion_cliente=disabled;ret_email_cliente=disabled", "");
