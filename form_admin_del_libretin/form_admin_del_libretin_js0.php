<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="lib_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['lib_id_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_admin_del_libretin"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?>
    ...
    {
        do_ajax_form_admin_del_libretin_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_form_admin_del_libretin_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_admin_del_libretin_submit_form(y, z);
