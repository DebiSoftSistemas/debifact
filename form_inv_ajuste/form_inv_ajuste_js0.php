<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="aj_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['aj_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_inv_ajuste']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_inv_ajuste']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_inv_ajuste"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_procesar = false;
function sc_btn_procesar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_procesar(); }, 500);
      return;
    }
    if (Crtl_btn_procesar) {return;}
    sc_btn_procesar_ok();
}
function sc_btn_procesar_cancel()
{
}
function sc_btn_procesar_ok()
{
    Crtl_btn_procesar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?procesar?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_inv_ajuste']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_inv_ajuste_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_inv_ajuste_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
?>
    ...
    {
        do_ajax_form_inv_ajuste_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_inv_ajuste_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
