...
<HEAD>
 <TITLE><?php if ('novo' == $this->nmgp_opcao) { echo strip_tags("" . $this->Ini->Nm_lang['lang_datos_empresa'] . ""); } else { echo strip_tags("" . $this->Ini->Nm_lang['lang_datos_empresa'] . ""); } ?></TITLE>
...
<script type="text/javascript" src="<?php echo $this->Ini->path_prod ?>/third/jquery_plugin/select2/js/select2.full.min.js"></script>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>form_del_nueva_empresa/form_del_nueva_empresa_mob_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("form_del_nueva_empresa_mob_sajax_js.php");
?>
<script type="text/javascript">
...
var Nav_binicio_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['first']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['first'] : 'off'); ?>";
var Nav_bavanca_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['forward']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['forward'] : 'off'); ?>";
var Nav_bretorna_macro_disabled = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['back']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['back'] : 'off'); ?>";
var Nav_bfinal_macro_disabled   = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['last']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['last'] : 'off'); ?>";
...
include_once('form_del_nueva_empresa_mob_jquery.php');
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['recarga'];
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
    $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['remove_border'] ? 'border-width: 0; ' : '';
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['link_info']['remove_margin']) {
        $remove_margin = 'margin: 0; ';
    }
    if ('' != $remove_margin && isset($str_iframe_body) && '' != $str_iframe_body) {
        $str_iframe_body = '';
    }
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['link_info']['remove_border']) {
...
if (isset($_SESSION['scriptcase']['form_del_nueva_empresa']['error_buffer']) && '' != $_SESSION['scriptcase']['form_del_nueva_empresa']['error_buffer'])
{
    echo $_SESSION['scriptcase']['form_del_nueva_empresa']['error_buffer'];
}
...
<?php
 include_once("form_del_nueva_empresa_mob_js0.php");
?>
<script type="text/javascript"> 
nmdg_enter_tab = true;
...
<form  name="F1" method="post" 
               action="form_del_nueva_empresa_mob.php" 
...
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['insert_validation'] = md5(time() . rand(1, 99999));
?>
<input type="hidden" name="nmgp_ins_valid" value="<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['insert_validation']; ?>">
<?php
}
?>
...
<?php
$_SESSION['scriptcase']['error_span_title']['form_del_nueva_empresa_mob'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['form_del_nueva_empresa_mob'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
   if (!isset($this->nmgp_cmp_hidden['emp_obligado']))
   {
       $this->nmgp_cmp_hidden['emp_obligado'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['emp_ambiente_sri']))
   {
       $this->nmgp_cmp_hidden['emp_ambiente_sri'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['usu_cedula']))
   {
       $this->nmgp_cmp_hidden['usu_cedula'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['emp_nombre_usuario']))
   {
       $this->nmgp_cmp_hidden['emp_nombre_usuario'] = 'off';
   }
   if (!isset($this->nmgp_cmp_hidden['usu_email']))
   {
       $this->nmgp_cmp_hidden['usu_email'] = 'off';
   }
?>
   ...
   <tr>
    ...
    <TD colspan="1" height="20" class="scFormBlock">
     <TABLE style="padding: 0px; spacing: 0px; border-width: 0px;" width="100%" height="100%">
      <TR>
       <TD align="" valign="" class="scFormBlockFont"><?php echo $this->Ini->Nm_lang['lang_datos_empresa'] ?></TD>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_emp_comisionista_line" id="hidden_field_data_emp_comisionista" style="<?php echo $sStyleHidden_emp_comisionista; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_emp_comisionista_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_emp_comisionista_label" style=""><span id="id_label_emp_comisionista"><?php echo $this->nm_new_label['emp_comisionista']; ?></span></span><br>
...
<?php
   ...
   echo " <span id=\"idAjaxSelect_emp_comisionista\" class=\"" . $this->classes_100perc_fields['span_select'] . "\"><select class=\"sc-js-input scFormObjectOdd css_emp_comisionista_obj" . $this->classes_100perc_fields['input'] . "\" style=\"\" id=\"id_sc_field_emp_comisionista\" name=\"emp_comisionista\" size=\"1\" alt=\"{type: 'select', enterTab: true}\">" ; 
   echo "\r" ; 
   $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['Lookup_emp_comisionista'][] = '1091786547001'; 
   echo "  <option value=\"1091786547001\">" . str_replace("<", "&lt;","" . $this->Ini->Nm_lang['lang_seleccione'] . "") . "</option>" ; 
...
<span id="id_read_on_emp_ruc" class="sc-ui-readonly-emp_ruc css_emp_ruc_line" style="<?php echo $sStyleReadLab_emp_ruc; ?>"><?php echo $this->form_format_readonly("emp_ruc", $this->form_encode_input($this->emp_ruc)); ?></span><span id="id_read_off_emp_ruc" class="css_read_off_emp_ruc<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_emp_ruc; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=13"; } ?> maxlength=13 alt="{datatype: 'text', maxLength: 13, allowedChars: '<?php echo $this->allowedCharsCharset("0123456789") ?>', lettersCase: 'upper', enterTab: true, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
<span id="id_read_on_emp_email" class="sc-ui-readonly-emp_email css_emp_email_line" style="<?php echo $sStyleReadLab_emp_email; ?>"><?php echo $this->form_format_readonly("emp_email", $this->form_encode_input($this->emp_email)); ?></span><span id="id_read_off_emp_email" class="css_read_off_emp_email<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_emp_email; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=50"; } ?> maxlength=100 alt="{enterTab: true, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" >&nbsp;<?php if ($this->nmgp_opcao != "novo"){ ?><?php echo nmButtonOutput($this->arr_buttons, "bemail", "if (document.F1.emp_email.value != '') {window.open('mailto:' + document.F1.emp_email.value); }", "if (document.F1.emp_email.value != '') {window.open('mailto:' + document.F1.emp_email.value); }", "emp_email_mail", "", "", "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
<?php } ?>
</span><?php } ?>
...
?> 
    ...
    <TD class="scFormDataOdd css_emp_obligado_line" id="hidden_field_data_emp_obligado" style="<?php echo $sStyleHidden_emp_obligado; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_emp_obligado_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_emp_obligado_label" style=""><span id="id_label_emp_obligado"><?php echo $this->nm_new_label['emp_obligado']; ?></span></span><br>
...
<span id="id_read_on_emp_obligado" class="css_emp_obligado_line" style="<?php echo $sStyleReadLab_emp_obligado; ?>"><?php echo $this->form_format_readonly("emp_obligado", $this->form_encode_input($emp_obligado_look)); ?></span><span id="id_read_off_emp_obligado" class="css_read_off_emp_obligado css_emp_obligado_line" style="<?php echo $sStyleReadInp_emp_obligado; ?>"><?php echo "<div id=\"idAjaxCheckbox_emp_obligado\" style=\"display: inline-block\" class=\"css_emp_obligado_line\">\r\n"; ?><TABLE cellspacing=0 cellpadding=0 border=0><TR>
 ...
 <div class="sc switch">
 <input type=checkbox id="<?php echo $tempOptionId ?>" class="sc-ui-checkbox-emp_obligado sc-ui-checkbox-emp_obligado sc-js-input" name="emp_obligado[]" value="S"
 alt="{type: 'checkbox', enterTab: true}"<?php $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['Lookup_emp_obligado'][] = 'S'; ?>
<?php  if (in_array("S", $this->emp_obligado_1))  { echo " checked" ;} ?> onClick="" ><span></span>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_emp_contribuyente_especial_line" id="hidden_field_data_emp_contribuyente_especial" style="<?php echo $sStyleHidden_emp_contribuyente_especial; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_emp_contribuyente_especial_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_emp_contribuyente_especial_label" style=""><span id="id_label_emp_contribuyente_especial"><?php echo $this->nm_new_label['emp_contribuyente_especial']; ?></span></span><br>
...
<span id="id_read_on_emp_contribuyente_especial" class="sc-ui-readonly-emp_contribuyente_especial css_emp_contribuyente_especial_line" style="<?php echo $sStyleReadLab_emp_contribuyente_especial; ?>"><?php echo $this->form_format_readonly("emp_contribuyente_especial", $this->form_encode_input($this->emp_contribuyente_especial)); ?></span><span id="id_read_off_emp_contribuyente_especial" class="css_read_off_emp_contribuyente_especial<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_emp_contribuyente_especial; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=5"; } ?> maxlength=5 alt="{datatype: 'text', maxLength: 5, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: true, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    ...
    {
        $this->nm_new_label['emp_ambiente_sri'] = "Emp Ambiente Sri";
    }
?>
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_emp_ambiente_sri_line" id="hidden_field_data_emp_ambiente_sri" style="<?php echo $sStyleHidden_emp_ambiente_sri; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_emp_ambiente_sri_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_emp_ambiente_sri_label" style=""><span id="id_label_emp_ambiente_sri"><?php echo $this->nm_new_label['emp_ambiente_sri']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["emp_ambiente_sri"]) &&  $this->nmgp_cmp_readonly["emp_ambiente_sri"] == "on") { 
 ...
 ?>
<input type="hidden" name="emp_ambiente_sri" value="<?php echo $this->form_encode_input($emp_ambiente_sri) . "\">" . $emp_ambiente_sri . ""; ?>
<?php } else { ?>
<span id="id_read_on_emp_ambiente_sri" class="sc-ui-readonly-emp_ambiente_sri css_emp_ambiente_sri_line" style="<?php echo $sStyleReadLab_emp_ambiente_sri; ?>"><?php echo $this->form_format_readonly("emp_ambiente_sri", $this->form_encode_input($this->emp_ambiente_sri)); ?></span><span id="id_read_off_emp_ambiente_sri" class="css_read_off_emp_ambiente_sri<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_emp_ambiente_sri; ?>">
 <input class="sc-js-input scFormObjectOdd css_emp_ambiente_sri_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_emp_ambiente_sri" type=text name="emp_ambiente_sri" value="<?php echo $this->form_encode_input($emp_ambiente_sri) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=9"; } ?> alt="{datatype: 'integer', maxLength: 9, thousandsSep: '<?php echo str_replace("'", "\'", $this->field_config['emp_ambiente_sri']['symbol_grp']); ?>', thousandsFormat: <?php echo $this->field_config['emp_ambiente_sri']['symbol_fmt']; ?>, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['emp_ambiente_sri']['format_neg'] ? "'suffix'" : "'prefix'") ?>, alignment: 'left', enterTab: true, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['emp_usuario']))
    {
...
<?php
   ...
   $sStyleHidden_emp_usuario = '';
   if (isset($this->nmgp_cmp_hidden['emp_usuario']) && $this->nmgp_cmp_hidden['emp_usuario'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['emp_usuario']);
       $sStyleHidden_emp_usuario = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_emp_usuario = 'display: none;';
   $sStyleReadInp_emp_usuario = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['emp_usuario']) && $this->nmgp_cmp_readonly['emp_usuario'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['emp_usuario']);
       $sStyleReadLab_emp_usuario = '';
       $sStyleReadInp_emp_usuario = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['emp_usuario']) && $this->nmgp_cmp_hidden['emp_usuario'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="emp_usuario" value="<?php echo $this->form_encode_input($emp_usuario) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_emp_usuario_line" id="hidden_field_data_emp_usuario" style="<?php echo $sStyleHidden_emp_usuario; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_emp_usuario_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_emp_usuario_label" style=""><span id="id_label_emp_usuario"><?php echo $this->nm_new_label['emp_usuario']; ?></span><?php if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['php_cmp_required']['emp_usuario']) || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['php_cmp_required']['emp_usuario'] == "on") { ?> <span class="scFormRequiredOdd">*</span> <?php }?></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["emp_usuario"]) &&  $this->nmgp_cmp_readonly["emp_usuario"] == "on") { 

 ?>
<input type="hidden" name="emp_usuario" value="<?php echo $this->form_encode_input($emp_usuario) . "\">" . $emp_usuario . ""; ?>
<?php } else { ?>
<span id="id_read_on_emp_usuario" class="sc-ui-readonly-emp_usuario css_emp_usuario_line" style="<?php echo $sStyleReadLab_emp_usuario; ?>"><?php echo $this->form_format_readonly("emp_usuario", $this->form_encode_input($this->emp_usuario)); ?></span><span id="id_read_off_emp_usuario" class="css_read_off_emp_usuario<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_emp_usuario; ?>">
 <input class="sc-js-input scFormObjectOdd css_emp_usuario_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_emp_usuario" type=text name="emp_usuario" value="<?php echo $this->form_encode_input($emp_usuario) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=20"; } ?> maxlength=35 alt="{datatype: 'text', maxLength: 35, allowedChars: '<?php echo $this->allowedCharsCharset("abcdefghijklmnopqrstuvwxyz0123456789ç.") ?>', lettersCase: '', enterTab: true, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_emp_usuario_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_emp_usuario_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['emp_clave']))
    {
...
<?php
   ...
   $sStyleHidden_emp_clave = '';
   if (isset($this->nmgp_cmp_hidden['emp_clave']) && $this->nmgp_cmp_hidden['emp_clave'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['emp_clave']);
       $sStyleHidden_emp_clave = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_emp_clave = 'display: none;';
   $sStyleReadInp_emp_clave = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['emp_clave']) && $this->nmgp_cmp_readonly['emp_clave'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['emp_clave']);
       $sStyleReadLab_emp_clave = '';
       $sStyleReadInp_emp_clave = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['emp_clave']) && $this->nmgp_cmp_hidden['emp_clave'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="emp_clave" value="<?php echo $this->form_encode_input($emp_clave) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_emp_clave_line" id="hidden_field_data_emp_clave" style="<?php echo $sStyleHidden_emp_clave; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_emp_clave_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_emp_clave_label" style=""><span id="id_label_emp_clave"><?php echo $this->nm_new_label['emp_clave']; ?></span><?php if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['php_cmp_required']['emp_clave']) || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['php_cmp_required']['emp_clave'] == "on") { ?> <span class="scFormRequiredOdd">*</span> <?php }?></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["emp_clave"]) &&  $this->nmgp_cmp_readonly["emp_clave"] == "on") { ?>
<input type="hidden" name="emp_clave" value="">
<?php } else { ?>
<span id="id_read_on_emp_clave" class="sc-ui-readonly-emp_clave css_emp_clave_line" style="<?php echo $sStyleReadLab_emp_clave; ?>"><?php echo $this->form_format_readonly("emp_clave", $this->form_encode_input($this->emp_clave)); ?></span><span id="id_read_off_emp_clave" class="css_read_off_emp_clave<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_emp_clave; ?>"><div class="scFormObjectOddPwdBox<?php echo $this->classes_100perc_fields['span_input'] ?>" style="width: 100%"><input class="sc-js-input scFormObjectOddPwdInput scFormObjectOddPwdText sc-ui-pwd-toggle css_emp_clave_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_emp_clave" type="password" autocomplete="off" name="emp_clave" value="" 
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=20"; } ?> maxlength=100 alt="{datatype: 'text', maxLength: 100, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: true, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ><span id="id_pwd_show_hide_emp_clave" class="sc-ui-pwd-toggle-icon"><i class="fa fa-eye sc-ui-pwd-eye" aria-hidden="true" id="id_pwd_fa_emp_clave"></i></span></div></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_emp_clave_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_emp_clave_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['emp_clave_reingreso']))
    {
...
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   $nm_img_fun_cel  = (isset($nm_img_fun_cel) && $nm_img_fun_cel  == $this->Ini->img_fun_imp    ? $this->Ini->img_fun_par  : $this->Ini->img_fun_imp);
   $emp_clave_reingreso = $this->emp_clave_reingreso;
   $sStyleHidden_emp_clave_reingreso = '';
   if (isset($this->nmgp_cmp_hidden['emp_clave_reingreso']) && $this->nmgp_cmp_hidden['emp_clave_reingreso'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['emp_clave_reingreso']);
       $sStyleHidden_emp_clave_reingreso = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_emp_clave_reingreso = 'display: none;';
   $sStyleReadInp_emp_clave_reingreso = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['emp_clave_reingreso']) && $this->nmgp_cmp_readonly['emp_clave_reingreso'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['emp_clave_reingreso']);
       $sStyleReadLab_emp_clave_reingreso = '';
       $sStyleReadInp_emp_clave_reingreso = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['emp_clave_reingreso']) && $this->nmgp_cmp_hidden['emp_clave_reingreso'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="emp_clave_reingreso" value="<?php echo $this->form_encode_input($emp_clave_reingreso) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_emp_clave_reingreso_line" id="hidden_field_data_emp_clave_reingreso" style="<?php echo $sStyleHidden_emp_clave_reingreso; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_emp_clave_reingreso_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_emp_clave_reingreso_label" style=""><span id="id_label_emp_clave_reingreso"><?php echo $this->nm_new_label['emp_clave_reingreso']; ?></span><?php if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['php_cmp_required']['emp_clave_reingreso']) || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['php_cmp_required']['emp_clave_reingreso'] == "on") { ?> <span class="scFormRequiredOdd">*</span> <?php }?></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["emp_clave_reingreso"]) &&  $this->nmgp_cmp_readonly["emp_clave_reingreso"] == "on") { ?>
<input type="hidden" name="emp_clave_reingreso" value="">
<?php } else { ?>
<span id="id_read_on_emp_clave_reingreso" class="sc-ui-readonly-emp_clave_reingreso css_emp_clave_reingreso_line" style="<?php echo $sStyleReadLab_emp_clave_reingreso; ?>"><?php echo $this->form_format_readonly("emp_clave_reingreso", $this->form_encode_input($this->emp_clave_reingreso)); ?></span><span id="id_read_off_emp_clave_reingreso" class="css_read_off_emp_clave_reingreso<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_emp_clave_reingreso; ?>"><div class="scFormObjectOddPwdBox<?php echo $this->classes_100perc_fields['span_input'] ?>" style="width: 100%"><input class="sc-js-input scFormObjectOddPwdInput scFormObjectOddPwdText sc-ui-pwd-toggle css_emp_clave_reingreso_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_emp_clave_reingreso" type="password" autocomplete="off" name="emp_clave_reingreso" value="" 
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=20"; } ?> maxlength=100 alt="{datatype: 'text', maxLength: 100, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: true, enterSubmit: false, autoTab: true, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ><span id="id_pwd_show_hide_emp_clave_reingreso" class="sc-ui-pwd-toggle-icon"><i class="fa fa-eye sc-ui-pwd-eye" aria-hidden="true" id="id_pwd_fa_emp_clave_reingreso"></i></span></div></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_emp_clave_reingreso_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_emp_clave_reingreso_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['new']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['new']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['new']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['new']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['new'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['insert']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['insert']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['insert']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['insert']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['insert'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['update']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['update']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['update']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['update']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['update'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label'][''];
        }
?>
<?php
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['help']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['help']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['help']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['help']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['help'];
        }
?>
...
<?php
    ...
    if ((!$this->Embutida_call || $this->form_3versions_single) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && $nm_apl_dependente != 1 && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe'] != "R" && !$this->aba_iframe && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
        $buttonMacroDisabled = 'sc-unique-btn-12';
        $buttonMacroLabel = "";

        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['exit']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_disabled']['exit']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['exit']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['exit']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['btn_label']['exit'];
        }
?>
...
<?php
    ...
    if ((!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (!isset($_SESSION['scriptcase']['nm_sc_retorno']) || empty($_SESSION['scriptcase']['nm_sc_retorno']) || $nm_apl_dependente == 1 || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe'] == "R" || $this->aba_iframe || $this->nmgp_botoes['exit'] != "on") && ($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['run_iframe'] != "F" && $this->nmgp_botoes['exit'] == "on") && ($nm_apl_dependente == 1 && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("form_del_nueva_empresa_mob");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("form_del_nueva_empresa_mob");
 parent.scAjaxDetailHeight("form_del_nueva_empresa_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
?>
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nueva_empresa_mob']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
