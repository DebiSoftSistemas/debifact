function grid_fac_factura_fac_numero_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_fac_numero" + seqRow).html();
}

function grid_fac_factura_fac_numero_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_numero" + seqRow).html(newValue);
}
function grid_fac_factura_fac_numero_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_fac_numero" + seqRow).html(newValue);
}

function grid_fac_factura_fac_serie_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_serie" + seqRow).html();
}

function grid_fac_factura_fac_serie_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_serie" + seqRow).html(newValue);
}

function grid_fac_factura_fac_numero_doc_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_numero_doc" + seqRow).html();
}

function grid_fac_factura_fac_numero_doc_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_numero_doc" + seqRow).html(newValue);
}

function grid_fac_factura_fac_cliente_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_cliente" + seqRow).html();
}

function grid_fac_factura_fac_cliente_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_cliente" + seqRow).html(newValue);
}

function grid_fac_factura_fac_fecha_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_fecha" + seqRow).html();
}

function grid_fac_factura_fac_fecha_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_fecha" + seqRow).html(newValue);
}

function grid_fac_factura_fac_subtotal_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_subtotal" + seqRow).html();
}

function grid_fac_factura_fac_subtotal_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_subtotal" + seqRow).html(newValue);
}

function grid_fac_factura_fac_descuento_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_descuento" + seqRow).html();
}

function grid_fac_factura_fac_descuento_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_descuento" + seqRow).html(newValue);
}

function grid_fac_factura_fac_total_iva_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_total_iva" + seqRow).html();
}

function grid_fac_factura_fac_total_iva_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_total_iva" + seqRow).html(newValue);
}

function grid_fac_factura_fac_total_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_total" + seqRow).html();
}

function grid_fac_factura_fac_total_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_total" + seqRow).html(newValue);
}

function grid_fac_factura_fac_estado_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_estado_sri" + seqRow).html();
}

function grid_fac_factura_fac_estado_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_estado_sri" + seqRow).html(newValue);
}

function grid_fac_factura_fac_forma_pago_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_forma_pago" + seqRow).html();
}

function grid_fac_factura_fac_forma_pago_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_forma_pago" + seqRow).html(newValue);
}

function grid_fac_factura_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("fac_numero" == field) {
    SC_tmp = grid_fac_factura_fac_numero_getValue(seqRow);
  }
  if ("fac_serie" == field) {
    SC_tmp = grid_fac_factura_fac_serie_getValue(seqRow);
  }
  if ("fac_numero_doc" == field) {
    SC_tmp = grid_fac_factura_fac_numero_doc_getValue(seqRow);
  }
  if ("fac_cliente" == field) {
    SC_tmp = grid_fac_factura_fac_cliente_getValue(seqRow);
  }
  if ("fac_fecha" == field) {
    SC_tmp = grid_fac_factura_fac_fecha_getValue(seqRow);
  }
  if ("fac_subtotal" == field) {
    SC_tmp = grid_fac_factura_fac_subtotal_getValue(seqRow);
  }
  if ("fac_descuento" == field) {
    SC_tmp = grid_fac_factura_fac_descuento_getValue(seqRow);
  }
  if ("fac_total_iva" == field) {
    SC_tmp = grid_fac_factura_fac_total_iva_getValue(seqRow);
  }
  if ("fac_total" == field) {
    SC_tmp = grid_fac_factura_fac_total_getValue(seqRow);
  }
  if ("fac_estado_sri" == field) {
    SC_tmp = grid_fac_factura_fac_estado_sri_getValue(seqRow);
  }
  if ("fac_forma_pago" == field) {
    SC_tmp = grid_fac_factura_fac_forma_pago_getValue(seqRow);
  }
...
function grid_fac_factura_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("fac_numero" == field) {
    grid_fac_factura_fac_numero_setValue(newValue, seqRow);
  }
  if ("fac_serie" == field) {
    grid_fac_factura_fac_serie_setValue(newValue, seqRow);
  }
  if ("fac_numero_doc" == field) {
    grid_fac_factura_fac_numero_doc_setValue(newValue, seqRow);
  }
  if ("fac_cliente" == field) {
    grid_fac_factura_fac_cliente_setValue(newValue, seqRow);
  }
  if ("fac_fecha" == field) {
    grid_fac_factura_fac_fecha_setValue(newValue, seqRow);
  }
  if ("fac_subtotal" == field) {
    grid_fac_factura_fac_subtotal_setValue(newValue, seqRow);
  }
  if ("fac_descuento" == field) {
    grid_fac_factura_fac_descuento_setValue(newValue, seqRow);
  }
  if ("fac_total_iva" == field) {
    grid_fac_factura_fac_total_iva_setValue(newValue, seqRow);
  }
  if ("fac_total" == field) {
    grid_fac_factura_fac_total_setValue(newValue, seqRow);
  }
  if ("fac_estado_sri" == field) {
    grid_fac_factura_fac_estado_sri_setValue(newValue, seqRow);
  }
  if ("fac_forma_pago" == field) {
    grid_fac_factura_fac_forma_pago_setValue(newValue, seqRow);
  }
  if ("fac_numero_Hidden" == field) {
    grid_fac_factura_fac_numero_Hidden_setValue(newValue, seqRow);
  }
}
...
function scAjaxCall_actbtn_btn_anular_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_anular_onClick&script_case_init=" + document.F3.script_case_init.value + "&fac_numero=" + grid_fac_factura_getValue("fac_numero", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_fac_factura_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
