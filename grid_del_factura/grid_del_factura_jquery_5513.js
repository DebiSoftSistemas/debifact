function grid_del_factura_fac_fecha_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_fecha" + seqRow).html();
}

function grid_del_factura_fac_fecha_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_fecha" + seqRow).html(newValue);
}

function grid_del_factura_sc_field_0_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_sc_field_0" + seqRow).html();
}

function grid_del_factura_sc_field_0_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_sc_field_0" + seqRow).html(newValue);
}

function grid_del_factura_fac_secuencial_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_secuencial" + seqRow).html();
}

function grid_del_factura_fac_secuencial_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_secuencial" + seqRow).html(newValue);
}

function grid_del_factura_fac_comentario_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_comentario" + seqRow).html();
}

function grid_del_factura_fac_comentario_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_comentario" + seqRow).html(newValue);
}

function grid_del_factura_cl_nombre_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_nombre" + seqRow).html();
}

function grid_del_factura_cl_nombre_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_nombre" + seqRow).html(newValue);
}

function grid_del_factura_cl_identificacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_identificacion" + seqRow).html();
}

function grid_del_factura_cl_identificacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_identificacion" + seqRow).html(newValue);
}

function grid_del_factura_fac_total_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_total" + seqRow).html();
}

function grid_del_factura_fac_total_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_total" + seqRow).html(newValue);
}

function grid_del_factura_fac_estado_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_estado" + seqRow).html();
}

function grid_del_factura_fac_estado_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_estado" + seqRow).html(newValue);
}

function grid_del_factura_fac_estado_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_estado_sri" + seqRow).html();
}

function grid_del_factura_fac_estado_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_estado_sri" + seqRow).html(newValue);
}

function grid_del_factura_fac_numero_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_fac_numero" + seqRow).html();
}

function grid_del_factura_fac_numero_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_numero" + seqRow).html(newValue);
}
function grid_del_factura_fac_numero_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_fac_numero" + seqRow).html(newValue);
}

function grid_del_factura_cl_direccion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_direccion" + seqRow).html();
}

function grid_del_factura_cl_direccion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_direccion" + seqRow).html(newValue);
}

function grid_del_factura_cl_email_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_email" + seqRow).html();
}

function grid_del_factura_cl_email_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_email" + seqRow).html(newValue);
}

function grid_del_factura_fac_autorizacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_autorizacion" + seqRow).html();
}

function grid_del_factura_fac_autorizacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_autorizacion" + seqRow).html(newValue);
}

function grid_del_factura_fac_error_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fac_error_sri" + seqRow).html();
}

function grid_del_factura_fac_error_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fac_error_sri" + seqRow).html(newValue);
}

function grid_del_factura_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("fac_fecha" == field) {
    SC_tmp = grid_del_factura_fac_fecha_getValue(seqRow);
  }
  if ("sc_field_0" == field) {
    SC_tmp = grid_del_factura_sc_field_0_getValue(seqRow);
  }
  if ("fac_secuencial" == field) {
    SC_tmp = grid_del_factura_fac_secuencial_getValue(seqRow);
  }
  if ("fac_comentario" == field) {
    SC_tmp = grid_del_factura_fac_comentario_getValue(seqRow);
  }
  if ("cl_nombre" == field) {
    SC_tmp = grid_del_factura_cl_nombre_getValue(seqRow);
  }
  if ("cl_identificacion" == field) {
    SC_tmp = grid_del_factura_cl_identificacion_getValue(seqRow);
  }
  if ("fac_total" == field) {
    SC_tmp = grid_del_factura_fac_total_getValue(seqRow);
  }
  if ("fac_estado" == field) {
    SC_tmp = grid_del_factura_fac_estado_getValue(seqRow);
  }
  if ("fac_estado_sri" == field) {
    SC_tmp = grid_del_factura_fac_estado_sri_getValue(seqRow);
  }
  if ("fac_numero" == field) {
    SC_tmp = grid_del_factura_fac_numero_getValue(seqRow);
  }
  if ("cl_direccion" == field) {
    SC_tmp = grid_del_factura_cl_direccion_getValue(seqRow);
  }
  if ("cl_email" == field) {
    SC_tmp = grid_del_factura_cl_email_getValue(seqRow);
  }
  if ("fac_autorizacion" == field) {
    SC_tmp = grid_del_factura_fac_autorizacion_getValue(seqRow);
  }
  if ("fac_error_sri" == field) {
    SC_tmp = grid_del_factura_fac_error_sri_getValue(seqRow);
  }
...
function grid_del_factura_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("fac_fecha" == field) {
    grid_del_factura_fac_fecha_setValue(newValue, seqRow);
  }
  if ("sc_field_0" == field) {
    grid_del_factura_sc_field_0_setValue(newValue, seqRow);
  }
  if ("fac_secuencial" == field) {
    grid_del_factura_fac_secuencial_setValue(newValue, seqRow);
  }
  if ("fac_comentario" == field) {
    grid_del_factura_fac_comentario_setValue(newValue, seqRow);
  }
  if ("cl_nombre" == field) {
    grid_del_factura_cl_nombre_setValue(newValue, seqRow);
  }
  if ("cl_identificacion" == field) {
    grid_del_factura_cl_identificacion_setValue(newValue, seqRow);
  }
  if ("fac_total" == field) {
    grid_del_factura_fac_total_setValue(newValue, seqRow);
  }
  if ("fac_estado" == field) {
    grid_del_factura_fac_estado_setValue(newValue, seqRow);
  }
  if ("fac_estado_sri" == field) {
    grid_del_factura_fac_estado_sri_setValue(newValue, seqRow);
  }
  if ("fac_numero" == field) {
    grid_del_factura_fac_numero_setValue(newValue, seqRow);
  }
  if ("cl_direccion" == field) {
    grid_del_factura_cl_direccion_setValue(newValue, seqRow);
  }
  if ("cl_email" == field) {
    grid_del_factura_cl_email_setValue(newValue, seqRow);
  }
  if ("fac_autorizacion" == field) {
    grid_del_factura_fac_autorizacion_setValue(newValue, seqRow);
  }
  if ("fac_error_sri" == field) {
    grid_del_factura_fac_error_sri_setValue(newValue, seqRow);
  }
  if ("fac_numero_Hidden" == field) {
    grid_del_factura_fac_numero_Hidden_setValue(newValue, seqRow);
  }
}
...
function scAjaxCall_actbtn_btn_anular_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_anular_onClick&script_case_init=" + document.F3.script_case_init.value + "&fac_numero=" + grid_del_factura_getValue("fac_numero", seqRow) + "&lang_doc_anulado=" + grid_del_factura_getValue("lang_doc_anulado", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_del_factura_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
...
function scAjaxCall_actbtn_btn_autorizar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_autorizar_onClick&script_case_init=" + document.F3.script_case_init.value + "&fac_numero=" + grid_del_factura_getValue("fac_numero", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      var i, fieldDisplay, oResp;
      Tst_integrid = jsonReturn.trim();
...
function ajax_navigate(opc, parm)
{
    ...
    return new Promise(function(resolve, reject) {$.ajax({
     ...
     .done(function(jsonNavigate) {
        ...
        if (!SC_Proc_Mobile) {
            document.getElementById('nmsc_iframe_liga_A_grid_del_factura').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_E_grid_del_factura').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_D_grid_del_factura').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_B_grid_del_factura').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_A_grid_del_factura').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_factura').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_factura').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_factura').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_A_grid_del_factura').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_factura').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_factura').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_factura').style.width  = '0px';
        }
        if (oResp["ajaxMessage"]) {
           nmAjaxMessage(oResp);
        }
