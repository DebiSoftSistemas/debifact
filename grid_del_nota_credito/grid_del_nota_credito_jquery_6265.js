function grid_del_nota_credito_nc_fecha_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nc_fecha" + seqRow).html();
}

function grid_del_nota_credito_nc_fecha_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nc_fecha" + seqRow).html(newValue);
}

function grid_del_nota_credito_sc_field_0_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_sc_field_0" + seqRow).html();
}

function grid_del_nota_credito_sc_field_0_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_sc_field_0" + seqRow).html(newValue);
}

function grid_del_nota_credito_nc_secuencial_docmod_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nc_secuencial_docmod" + seqRow).html();
}

function grid_del_nota_credito_nc_secuencial_docmod_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nc_secuencial_docmod" + seqRow).html(newValue);
}

function grid_del_nota_credito_cl_nombre_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_nombre" + seqRow).html();
}

function grid_del_nota_credito_cl_nombre_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_nombre" + seqRow).html(newValue);
}

function grid_del_nota_credito_cl_identificacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_identificacion" + seqRow).html();
}

function grid_del_nota_credito_cl_identificacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_identificacion" + seqRow).html(newValue);
}

function grid_del_nota_credito_nc_total_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nc_total" + seqRow).html();
}

function grid_del_nota_credito_nc_total_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nc_total" + seqRow).html(newValue);
}

function grid_del_nota_credito_nc_estado_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nc_estado_sri" + seqRow).html();
}

function grid_del_nota_credito_nc_estado_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nc_estado_sri" + seqRow).html(newValue);
}

function grid_del_nota_credito_pdf_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_pdf" + seqRow).find("i").attr("src");
}

function grid_del_nota_credito_pdf_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_pdf" + seqRow).find("img").attr("src", newValue);
}

function grid_del_nota_credito_xml_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_xml" + seqRow).find("i").attr("src");
}

function grid_del_nota_credito_xml_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_xml" + seqRow).find("img").attr("src", newValue);
}

function grid_del_nota_credito_cl_direccion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_direccion" + seqRow).html();
}

function grid_del_nota_credito_cl_direccion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_direccion" + seqRow).html(newValue);
}

function grid_del_nota_credito_nc_autorizacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nc_autorizacion" + seqRow).html();
}

function grid_del_nota_credito_nc_autorizacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nc_autorizacion" + seqRow).html(newValue);
}

function grid_del_nota_credito_nc_error_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nc_error_sri" + seqRow).html();
}

function grid_del_nota_credito_nc_error_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nc_error_sri" + seqRow).html(newValue);
}

function grid_del_nota_credito_nc_numero_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nc_numero" + seqRow).html();
}

function grid_del_nota_credito_nc_numero_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nc_numero" + seqRow).html(newValue);
}

function grid_del_nota_credito_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("nc_fecha" == field) {
    SC_tmp = grid_del_nota_credito_nc_fecha_getValue(seqRow);
  }
  if ("sc_field_0" == field) {
    SC_tmp = grid_del_nota_credito_sc_field_0_getValue(seqRow);
  }
  if ("nc_secuencial_docmod" == field) {
    SC_tmp = grid_del_nota_credito_nc_secuencial_docmod_getValue(seqRow);
  }
  if ("cl_nombre" == field) {
    SC_tmp = grid_del_nota_credito_cl_nombre_getValue(seqRow);
  }
  if ("cl_identificacion" == field) {
    SC_tmp = grid_del_nota_credito_cl_identificacion_getValue(seqRow);
  }
  if ("nc_total" == field) {
    SC_tmp = grid_del_nota_credito_nc_total_getValue(seqRow);
  }
  if ("nc_estado_sri" == field) {
    SC_tmp = grid_del_nota_credito_nc_estado_sri_getValue(seqRow);
  }
  if ("pdf" == field) {
    SC_tmp = grid_del_nota_credito_pdf_getValue(seqRow);
  }
  if ("xml" == field) {
    SC_tmp = grid_del_nota_credito_xml_getValue(seqRow);
  }
  if ("cl_direccion" == field) {
    SC_tmp = grid_del_nota_credito_cl_direccion_getValue(seqRow);
  }
  if ("nc_autorizacion" == field) {
    SC_tmp = grid_del_nota_credito_nc_autorizacion_getValue(seqRow);
  }
  if ("nc_error_sri" == field) {
    SC_tmp = grid_del_nota_credito_nc_error_sri_getValue(seqRow);
  }
  if ("nc_numero" == field) {
    SC_tmp = grid_del_nota_credito_nc_numero_getValue(seqRow);
  }
...
function grid_del_nota_credito_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("nc_fecha" == field) {
    grid_del_nota_credito_nc_fecha_setValue(newValue, seqRow);
  }
  if ("sc_field_0" == field) {
    grid_del_nota_credito_sc_field_0_setValue(newValue, seqRow);
  }
  if ("nc_secuencial_docmod" == field) {
    grid_del_nota_credito_nc_secuencial_docmod_setValue(newValue, seqRow);
  }
  if ("cl_nombre" == field) {
    grid_del_nota_credito_cl_nombre_setValue(newValue, seqRow);
  }
  if ("cl_identificacion" == field) {
    grid_del_nota_credito_cl_identificacion_setValue(newValue, seqRow);
  }
  if ("nc_total" == field) {
    grid_del_nota_credito_nc_total_setValue(newValue, seqRow);
  }
  if ("nc_estado_sri" == field) {
    grid_del_nota_credito_nc_estado_sri_setValue(newValue, seqRow);
  }
  if ("pdf" == field) {
    grid_del_nota_credito_pdf_setValue(newValue, seqRow);
  }
  if ("xml" == field) {
    grid_del_nota_credito_xml_setValue(newValue, seqRow);
  }
  if ("cl_direccion" == field) {
    grid_del_nota_credito_cl_direccion_setValue(newValue, seqRow);
  }
  if ("nc_autorizacion" == field) {
    grid_del_nota_credito_nc_autorizacion_setValue(newValue, seqRow);
  }
  if ("nc_error_sri" == field) {
    grid_del_nota_credito_nc_error_sri_setValue(newValue, seqRow);
  }
  if ("nc_numero" == field) {
    grid_del_nota_credito_nc_numero_setValue(newValue, seqRow);
  }
}
...
function scAjaxCall_actbtn_btn_anular_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_anular_onClick&script_case_init=" + document.F3.script_case_init.value + "&nc_numero=" + grid_del_nota_credito_getValue("nc_numero", seqRow) + "&lang_doc_anulado=" + grid_del_nota_credito_getValue("lang_doc_anulado", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_del_nota_credito_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
...
function scAjaxCall_actbtn_btn_autorizar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_autorizar_onClick&script_case_init=" + document.F3.script_case_init.value + "&nc_numero=" + grid_del_nota_credito_getValue("nc_numero", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      var i, fieldDisplay, oResp;
      Tst_integrid = jsonReturn.trim();
...
function ajax_navigate(opc, parm)
{
    ...
    return new Promise(function(resolve, reject) {$.ajax({
     ...
     .done(function(jsonNavigate) {
        ...
        if (!SC_Proc_Mobile) {
            document.getElementById('nmsc_iframe_liga_A_grid_del_nota_credito').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_E_grid_del_nota_credito').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_D_grid_del_nota_credito').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_B_grid_del_nota_credito').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_A_grid_del_nota_credito').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_nota_credito').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_nota_credito').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_nota_credito').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_A_grid_del_nota_credito').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_del_nota_credito').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_del_nota_credito').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_del_nota_credito').style.width  = '0px';
        }
        if (oResp["ajaxMessage"]) {
           nmAjaxMessage(oResp);
        }
